$( document ).ready(function() {
 ...
 var id_current_plan_estudio=8; // inicia con el id plan estudio de dgb
 var id_current_corporacion=8; // inicia con el id_corporacion  de dgb
  ...
  var id_current_post=0;
		...
		var data_ajax2= {
					...
					GetAjax(data_ajax2,"GetCourses",function(response){
						...
						if(courses[0]!=undefined){
								...
								$("#f_id_corporacion").val(id_current_plan_estudio);
	...
	$("#id_course").change(function() { // cuando cambia el select del plan de estudio optiene las materias
		...
		$("#f_u_id_plan_estudio").val(id_current_plan_estudio);
		$("#f_u_id_corporacion").val(id_current_plan_estudio);
		var data_ajax2= {
...
/*====  CLICKS POSTS */
	...
	$("#btn_form_post_delete").click(function(){
		SendAjax(
			"form_post_delete",
			"DeletePost",
			function(){
...
/*===================*/
	...
	function RefreshForums(forums_){
		...
		if(forums_.length==0 || forums_==null || forums_===undefined){
			...
			for (var i = 0; i < forums_.length; i++) {
				html_forums+='<div  class="list-group-item">';
	            html_forums+='	<a href="#"  class="threads_forum" item-index="'+i+'"  >'+forums_[i]['nombre']+'</a>'; 
	            html_forums+='	<div class="pull-right">';
	            ...
	            html_forums+='		<a href="#"  class="threads_forum" item-index="'+i+'"  > <span class="glyphicon glyphicon-align-justify" ></span></a>';
	            html_forums+='	</div>';
	            html_forums+='</div>';
			}			
		}
	...
	function RefreshThreads(threads_){
		...
		if(threads_.length==0 || threads_ ==null || threads_===undefined){
            ...
            html_threads+='	<div class="alert alert-default" > No hay preguntas detonadoras registradas hasta este momento.</div>'; 
			...
			for (var i = 0; i < threads_.length; i++) {
	            ...
	            html_threads+='  <h6 class="list-group-item-heading"><span class="glyphicon glyphicon-comment" ></span> '+threads_[i]['nombre']+'</h6>'; 
	            html_threads+='	<div class="pull-right">';
	            ...
	            html_threads+='	</div>';
	            html_threads+='		<p class="list-group-item-text"><a href="#" class="posts_thread" item-index="'+i+'"  > Comentarios <small>('+threads_[i]['comentarios']+')</small></a></p>';
	            html_threads+='</div>';
			}			
		}
	...
	function RefreshPosts(posts_){
		...
		if(posts_.length==0 || posts_==null || posts_===undefined){
				...
				if( threads[idx_current_thread]['id_admin'] != id_persona){
					...
					console.log("ES ADMIN"+id_persona+" | "+threads[idx_current_thread]['id_admin'] );
				}
			            	...
			            	if(respuestas[j]['texto']==posts_[alreadyPostedIdx]['respuesta_text']){
			            		var html_resp_user="";
				            		if(respuestas[j]['es_correcta']==1){
				            			html_resp_user='<label class="label label-success" data-toggle="tooltip" title="CORRECTA" ><span  class="glyphicon glyphicon-ok-sign" ></span></label></strong>';
				            		}else{
				            			html_resp_user='<label class="label label-danger" data-toggle="tooltip" title="INCORRECTA" ><span  class="glyphicon glyphicon-remove-sign" ></span></label></strong>';
				            		}
 								html_posts+='			<div class="col-md-12 text-left" >'
						...
						if(posts_[i]['user']==null || posts_[i]['user']==""){
							...
							if(threads[idx_current_thread]['tipo'] == 2){ // si es de tipo multiRESPUESTA
								if(posts_[i]['respuesta_correcta']==1){ // el usuario dio una respuesta correcta
									html_posts+='		<p >'+posts_[i]['fecha_registro']+' - <strong> '+user+'</strong> : <label class="label label-success" data-toggle="tooltip" title="CORRECTA: '+posts_[i]['respuesta_text']+'" ><span  class="glyphicon glyphicon-ok-sign" ></span></label><a href="#" class="btn btn-danger btn_post_delete" item-index="'+i+'" >Eliminar</a></p>';
								}else{ // el usuario dio una respuesta incorrecta
									html_posts+='		<p >'+posts_[i]['fecha_registro']+' - <strong> '+user+'</strong> : <label class="label label-danger" data-toggle="tooltip" title="INCORRECTA: '+posts_[i]['respuesta_text']+'" ><span  class="glyphicon glyphicon-remove-sign" ></span></label><a href="#" class="btn btn-danger btn_post_delete" item-index="'+i+'" >Eliminar</a></p>';
								}
							}else{ // no es un hilo de multirespuesta, ES UN hilo normal
								html_posts+='		<p >'+posts_[i]['fecha_registro']+' -<strong> '+user+'</strong><a href="#" class="btn btn-danger btn_post_delete" item-index="'+i+'" >Eliminar</a></p>';
							}
		...
		$(".btn_post_delete").click(function(){
			var index=parseInt($(this).attr("item-index"));
			idx_current_post=index;
			

			$("#p_d_id_post").val(posts[idx_current_post]['id']);
			$("#p_d_nombre").val(posts[idx_current_post]['texto']);
