<?php 
	class Usuarios extends Connection {
		...
		function GetUsuarios($bd="",$id_plan_estudio=0,$id_moodle=0,$numero_empleado="null",$nombre=""){
			...
			$query="SELECT tp.*,ta.idmoodle,ta.numero_empleado,ta.id_plan_estudio,m.username,concat(m.firstname,' ',m.lastname) as nombre_plataforma,ta.tipo, m.email FROM escolar.tb_personas tp 
		...
		function UpdateUsuario($bd="",$id_plan_estudio=0,$id_moodle=0,$id_persona=0,$nombre="",$apellido1="",$apellido2="",$usuario="nodefinio",$numero_empleado,$fecha_nacimiento="0000-00-00",$tipo_alumno=1,$numero_sucursal='',$nombre_sucursal='',$horario='',$id_estado=1,$sexo=0,$email='noemail@agcollege.edu.mx',$id_sucursal=0){
			...
			$query_tb_personas="UPDATE escolar.tb_personas tp SET tp.sexo='".$sexo."',tp.fecha_nacimiento='".$fecha_nacimiento."', tp.contrasena='".$contra."', tp.usuario='".$usuario."', tp.nombre='".$nombre."', tp.apellido1='".$apellido1."', tp.apellido2='".$apellido2."',tp.sucursal='".$id_sucursal."',tp.estado='".$id_estado."' WHERE tp.id='".$id_persona."' limit 1 " ;
