<?php
		...
		$id_plan_estudio_columnas = 13;

		array_push($columns_tmp,new Column(3,1,"ingreso","ingreso","IFNULL((select ta.fecha_inscripcion 
				from escolar.tb_alumnos ta where ta.idmoodle=mdl_user.id AND ta.id_plan_estudio = 13 limit 1),' ')", "mdl_user","Fecha Inscripcion") );
	...
	array_push($columns_tmp,new Column(130,1,"responsable_admision","responsable_admision","IFNULL((select IF(escolar.tb_alumnos.id_usuario_responsable = 210, 'NO DEFINIDO', concat(usu.nombre,' ', ifnull(usu.apellidop,''),' ', ifnull(usu.apellidom,''))) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
				where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=13 limit 1),'')", "mdl_user","Responsable Admisión Inscripción") );	
				...
				WHERE escolar.tb_alumnos.idmoodle = mdl_user.id  AND escolar.tb_alumnos.id_plan_estudio = 13 LIMIT 1),'NO DEFINIDO')", "mdl_user","Responsable Gestión Escolar Inscripción") );
...
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=13 limit 1),1)
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
inner join escolar.tb_alumnos_estados on escolar.tb_alumnos_estados.id=escolar.tb_alumnos.estado
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=13 limit 1),' ')", "mdl_user","Descripcion status") );
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona and  escolar.tb_alumnos.id_plan_estudio = 13
inner join escolar.tb_alumnos_estados_log on escolar.tb_alumnos_estados_log.id_alumno=escolar.tb_alumnos.id
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=13 order by escolar.tb_alumnos_estados_log.fecha DESC limit 1),' ')", "mdl_user","Fecha status") );
	...
	*/
		...
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=13 limit 1),' ')", "mdl_user","#Empleado",true) );	
		...
		*/
		array_push($columns_tmp,new Column(7,1,"region","region","(select tb_regiones.nombre from escolar.tb_alumnos inner join escolar.tb_personas on tb_personas.id = tb_alumnos.id_persona and tb_alumnos.id_plan_estudio = 13 inner join escolar.tb_regiones on tb_regiones.id = tb_personas.region where tb_alumnos.idmoodle = mdl_user.id limit 1) ", "mdl_user","Region") );
		
		$columna= new Column(13,2,"data","materias_sumate","SELECT mdl_course.id,mdl_course.fullname,mdl_course_categories.name FROM mdl_course inner join mdl_course_categories on mdl_course.category=mdl_course_categories.id WHERE mdl_course_categories.parent=4 order by mdl_course_categories.name ASC,mdl_course.id ASC", "mdl_user","Materias");
	...
	$columna2= new Column(133,2,"data","ex_materias_sumate","SELECT mdl_course.id,mdl_course.fullname,mdl_course_categories.name FROM mdl_course inner join mdl_course_categories on mdl_course.category=mdl_course_categories.id WHERE mdl_course_categories.parent=4 order by mdl_course_categories.name ASC,mdl_course.id ASC", "mdl_user","Exame. Materias");
...
mdl_scorm_scoes_track.`value`='failed' )) ";
		...
		array_push($columns_tmp,new Column(15,1,"numero_telefonico","numero_telefonico","IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt inner join escolar.tb_alumnos ta on tnt.id_alumno=ta.id WHERE  tnt.eliminado=0 and ta.idmoodle=mdl_user.id and ta.id_plan_estudio=13  order by tnt.id DESC limit 1),'SIN NUMERO')", "mdl_user","Numero Telefonico") );
		...
		$querisillo = "
			...
			FROM
				escolar.tb_alumnos
				INNER JOIN escolar.tb_personas ON tb_personas.id = tb_alumnos.id_persona AND tb_alumnos.id_plan_estudio = 13
		...
		array_push($columns_tmp,new Column(23, 1, "fecha_nacimiento", "fecha_nacimiento", "(".$querisillo.")", "mdl_user", "FECHA DE NACIMIENTO"));
			...
			array_push($columns_tmp,new Column(998,1,"periodo_inscripcion","periodo_inscripcion","IFNULL((select periodo_inscripcion from escolar.tb_alumnos where id_plan_estudio = 13 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user","Periodo Inscripcion"));
/*Id
...
*/
	array_push($columns_tmp, new Column(
		...
		"promedio_general", 
		"promedio_general", 
		"IFNULL((SELECT COALESCE(FORMAT((SUM(IF(f.calificacion > 10, (f.calificacion * 0.1), f.calificacion))/COUNT(id)), 2), 0) FROM prepasumate.ag_calificaciones f WHERE f.id_alumno = mdl_user.id) ,' ')", 
		"mdl_user",
		"Promedio General")
	);
	...
	array_push($columns_tmp, new Column(
		...
		"periodo_actual", 
		"periodo_actual", 
		"COALESCE((SELECT MAX(j.periodo) FROM prepasumate.mdl_user_enrolments g INNER JOIN prepasumate.mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = 13 INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = mdl_user.id), 0)", 
		"mdl_user", 
		"Periodo Actual")
	);
