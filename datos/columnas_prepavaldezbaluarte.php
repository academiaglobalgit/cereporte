<?php
	...
	array_push($columns_tmp,new Column(3,1,"firstname","firstname","IFNULL((select tp.nombre 
				...
				inner join escolar.tb_alumnos ta on tp.id = ta.id_persona and  ta.id_plan_estudio = 74		
	...
	array_push($columns_tmp,new Column(130,1,"responsable_inscripcion","responsable_inscripcion","IFNULL((select concat(usu.nombre,' ', usu.apellidop,' ', usu.apellidom) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
				where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=74 limit 1),'')", "mdl_user","Responsable Inscripcion") );
