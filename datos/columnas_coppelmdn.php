<?php 
	...
	$plan_estudio = 50;
	$id_plan_estudio_columnas = 50;
	/*AGREGAR COLUMNAS PERSONALIZADAS */

	array_push($columns_tmp,new Column(3, 1, "ingreso", "ingreso", "IFNULL((select ta.fecha_inscripcion 
		from escolar.tb_alumnos ta where ta.idmoodle=mdl_user.id AND ta.id_plan_estudio= $plan_estudio limit 1),' ')", "mdl_user","Fecha Inscripcion") );
	...
	array_push($columns_tmp,new Column(3, 1, "firstname", "firstname", "IFNULL((select tp.nombre 
		...
		AND ta.id_plan_estudio= $plan_estudio
	...
	array_push($columns_tmp,new Column(130,1,"responsable_admision","responsable_admision","IFNULL((select IF(escolar.tb_alumnos.id_usuario_responsable = 210, 'NO DEFINIDO', concat(usu.nombre,' ', ifnull(usu.apellidop,''),' ', ifnull(usu.apellidom,''))) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=$plan_estudio limit 1),'')", "mdl_user","Responsable Admisión Inscripción") );	
		...
		WHERE escolar.tb_alumnos.idmoodle = mdl_user.id  AND escolar.tb_alumnos.id_plan_estudio = $plan_estudio LIMIT 1),'NO DEFINIDO')", "mdl_user","Responsable Gestión Escolar Inscripción") );
	//columna tipo 0  ()
	...
	array_push($columns_tmp,new Column(4,1,"tipo_status","tipo_status","IFNULL((select escolar.tb_alumnos_estados.descripcion from escolar.tb_personas 
		...
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio= $plan_estudio limit 1),' ')", "mdl_user","Descripcion status") );
	
	array_push($columns_tmp,new Column(117,1,"motivo_baja","motivo_baja","IFNULL((select 
		...
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=$plan_estudio limit 1),' ')", "mdl_user","Motivo Baja") );
	...
	array_push($columns_tmp,new Column(5,1,"suspended","Status","IFNULL((select escolar.tb_alumnos_estados.nombre_categoria from escolar.tb_personas 
		inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
		inner join escolar.tb_alumnos_estados on escolar.tb_alumnos_estados.id=escolar.tb_alumnos.estado
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio= $plan_estudio limit 1), 1)
	...
	array_push($columns_tmp,new Column(6,1,"n_empleado","n_empleado","IFNULL((select escolar.tb_alumnos.numero_empleado from escolar.tb_alumnos
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio= $plan_estudio limit 1),' ')", "mdl_user","#Empleado",true) );	
	...
	array_push($columns_tmp,new Column(14,1,"fecha_status","fecha_status","IFNULL((select escolar.tb_alumnos_estados_log.fecha from escolar.tb_personas 
		inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona and  escolar.tb_alumnos.id_plan_estudio = $plan_estudio
		inner join escolar.tb_alumnos_estados_log on escolar.tb_alumnos_estados_log.id_alumno=escolar.tb_alumnos.id
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio= $plan_estudio order by escolar.tb_alumnos_estados_log.fecha DESC limit 1),' ')", "mdl_user","Fecha status",false) );
	...
	array_push($columns_tmp,new Column(10,1,"telefonos","telefonos","(select group_concat(numero_telefonico,' - ') from escolar.tb_numeros_telefonicos inner join escolar.tb_alumnos on tb_alumnos.id = tb_numeros_telefonicos.id_alumno where tb_alumnos.id_plan_estudio = $plan_estudio and tb_alumnos.idmoodle = mdl_user.id group by tb_alumnos.id) ", "mdl_user","Teléfonos") );
	...
	$querisillo = "
		...
		FROM
			escolar.tb_alumnos
			INNER JOIN escolar.tb_personas ON tb_personas.id = tb_alumnos.id_persona AND tb_alumnos.id_plan_estudio = $plan_estudio
	...
	$querisillo = "
		...
		WHERE
			...
			AND escolar.tb_alumnos.id_plan_estudio = $plan_estudio
	...
	array_push($columns_tmp, new Column(27, 1, "periodo_inscripcion", "periodo_inscripcion", "IFNULL((select periodo_inscripcion from escolar.tb_alumnos where id_plan_estudio = $plan_estudio and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Periodo Inscripcion"));

	array_push($columns_tmp,new Column(28, 1, "generacion_actual", "generacion_actual","IFNULL((SELECT id_generacion_academica from escolar.tb_alumnos where id_plan_estudio = $plan_estudio and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Generación Actual"));
	...
	array_push($columns_tmp, new Column(
		...
		"promedio_general", 
		"promedio_general", 
		"IFNULL((SELECT COALESCE(FORMAT((SUM(IF(f.calificacion > 10, (f.calificacion * 0.1), f.calificacion))/COUNT(id)), 2), 0) FROM ag_calificaciones f WHERE f.id_alumno = mdl_user.id) ,' ')", 
		"mdl_user", 
		"Promedio General"));
	...
	array_push($columns_tmp, new Column(
		...
		"periodo_actual", 
		"periodo_actual", 
		"COALESCE((SELECT MAX(j.periodo) FROM mdl_user_enrolments g INNER JOIN mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = $plan_estudio INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = mdl_user.id), 0)", 
		"mdl_user", 
		"Periodo Actual"));
	...
	array_push($columns_tmp,new Column(
		...
		"numero_telefonico",
		"numero_telefonico",
		"IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt inner join escolar.tb_alumnos ta on tnt.id_alumno=ta.id WHERE  tnt.eliminado=0 and ta.idmoodle=mdl_user.id and ta.id_plan_estudio= $plan_estudio  order by tnt.id DESC limit 1),'SIN NUMERO')", 
		"mdl_user",
		"Numero Telefonico") );

	array_push($columns_tmp, new Column(32, 1, "correo_electronico_escolar", "correo_electronico_escolar", "IFNULL((SELECT tb_personas.email FROM escolar.tb_personas INNER JOIN escolar.tb_alumnos ON tb_alumnos.id_persona = tb_personas.id WHERE tb_alumnos.id_plan_estudio = $plan_estudio AND tb_alumnos.idmoodle = mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Correo Electronico Escolar"));
	...
	array_push($columns_tmp,new Column(
		...
		"mensualidad_activa",
		"IFNULL((SELECT
				IF(ma.estado = 1,'SI','NO') AS 'mensualidad_activa'
			FROM
				escolar.tb_alumnos ta 
				INNER JOIN escolar.tb_alumnos_mensualidad_activa ma ON ma.id_alumno = ta.id
			WHERE
				ta.idmoodle = mdl_user.id
			LIMIT 1), 'NO')", 
		"mdl_user",
