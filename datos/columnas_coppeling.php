<?php 
	...
	$id_plan_estudio_columnas = 18;

	array_push($columns_tmp,new Column(3, 1, "ingreso", "ingreso", "IFNULL((select ta.fecha_inscripcion 
		from escolar.tb_alumnos ta where ta.idmoodle=mdl_user.id AND ta.id_plan_estudio=18 limit 1),' ')", "mdl_user","Fecha Inscripcion") );
	...
	array_push($columns_tmp,new Column(1000,1,"responsable_admision","responsable_admision","IFNULL((select IF(escolar.tb_alumnos.id_usuario_responsable = 210, 'NO DEFINIDO', concat(usu.nombre,' ', ifnull(usu.apellidop,''),' ', ifnull(usu.apellidom,''))) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=18 limit 1),'')", "mdl_user","Responsable Admisión Inscripción") );	
		array_push($columns_tmp,new Column(1001,1,"responsable_gestion","responsable_gestion","IFNULL((select IF (log.id = 210, 'NO DEFINIDO', concat(usu.nombre, ' ', ifnull( usu.apellidop, '' ), ' ', ifnull( usu.apellidom, '' ))) FROM escolar.tb_alumnos LEFT JOIN escolar.tb_alumnos_a1 log on log.id = escolar.tb_alumnos.id_alumno_a1 LEFT JOIN escolar.tb_usuarios usu ON usu.id = log.id_usuario_registra 
		WHERE escolar.tb_alumnos.idmoodle = mdl_user.id  AND escolar.tb_alumnos.id_plan_estudio = 18 LIMIT 1),'NO DEFINIDO')", "mdl_user","Responsable Gestión Escolar Inscripción") );
		//columna tipo 0  ()
	...
	array_push($columns_tmp,new Column(4,1,"tipo_status","tipo_status","IFNULL((select escolar.tb_alumnos_estados.descripcion from escolar.tb_personas 
		...
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=18 limit 1),' ')", "mdl_user","Descripcion status") );
	
	array_push($columns_tmp,new Column(120,1,"motivo_baja","motivo_baja","IFNULL((select 
		...
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=18 limit 1),' ')", "mdl_user","Motivo Baja") );
	...
	array_push($columns_tmp,new Column(5,1,"suspended","Status","IFNULL((select escolar.tb_alumnos_estados.nombre_categoria from escolar.tb_personas 
		inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
		inner join escolar.tb_alumnos_estados on escolar.tb_alumnos_estados.id=escolar.tb_alumnos.estado
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=18 limit 1), 1)
	...
	array_push($columns_tmp,new Column(6,1,"n_empleado","n_empleado","IFNULL((select escolar.tb_alumnos.numero_empleado from escolar.tb_alumnos
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=18 limit 1),' ')", "mdl_user","#Empleado",true) );	
	...
	array_push($columns_tmp,new Column(14,1,"fecha_status","fecha_status","IFNULL((select escolar.tb_alumnos_estados_log.fecha from escolar.tb_personas 
		inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona and  escolar.tb_alumnos.id_plan_estudio = 18
		inner join escolar.tb_alumnos_estados_log on escolar.tb_alumnos_estados_log.id_alumno=escolar.tb_alumnos.id
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=18 order by escolar.tb_alumnos_estados_log.fecha DESC limit 1),' ')", "mdl_user","Fecha status",false) );
	...
	// array_push($columns_tmp,new Column(10,1,"telefonos","telefonos","(select group_concat(numero_telefonico,' - ') from escolar.tb_numeros_telefonicos inner join escolar.tb_alumnos on tb_alumnos.id = tb_numeros_telefonicos.id_alumno where tb_alumnos.id_plan_estudio = 18 and tb_alumnos.idmoodle = mdl_user.id group by tb_alumnos.id) ", "mdl_user","Teléfonos") );
	...
	$querisillo = "
		...
		FROM
			escolar.tb_alumnos
			INNER JOIN escolar.tb_personas ON tb_personas.id = tb_alumnos.id_persona AND tb_alumnos.id_plan_estudio = 18
			INNER JOIN escolar.tb_regiones ON tb_regiones.id = tb_personas.region
		WHERE
			tb_alumnos.idmoodle = mdl_user.id
		LIMIT 1
	";
	array_push($columns_tmp,new Column(24, 1, "curp", "curp", "(".$querisillo.")", "mdl_user", "CURP"));
	...
	$querisillo = "
		...
		LIMIT 1
	";
	array_push($columns_tmp,new Column(25, 1, "fecha_nacimiento", "fecha_nacimiento", "(".$querisillo.")", "mdl_user", "FECHA DE NACIMIENTO"));
	...
	$querisillo = "
		...
		WHERE
			...
			AND escolar.tb_alumnos.id_plan_estudio = 18
			AND escolar.tb_peticiones.id_estado = 1
		LIMIT 1
	";
	array_push($columns_tmp, new Column(26, 1, "peticiones_pendientes", "peticiones_pendientes", "(".$querisillo.")", "mdl_user", "PETICIONES PENDIENTES"));

	array_push($columns_tmp, new Column(26, 1, "periodos_terminados", "periodos_terminados", "(select rv_get_periodo_actual(mdl_user.id) )", "mdl_user", "Periodos Terminados") );


	array_push($columns_tmp, new Column(27, 1, "periodo_inscripcion", "periodo_inscripcion", "IFNULL((select periodo_inscripcion from escolar.tb_alumnos where id_plan_estudio = 18 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Periodo Inscripcion"));

	array_push($columns_tmp,new Column(28, 1, "generacion_actual", "generacion_actual","IFNULL((SELECT id_generacion_academica from escolar.tb_alumnos where id_plan_estudio = 18 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Generación Actual"));
	...
	array_push($columns_tmp, new Column(
		...
		"promedio_general", 
		"promedio_general", 
		"IFNULL((SELECT COALESCE(FORMAT((SUM(IF(f.calificacion > 10, (f.calificacion * 0.1), f.calificacion))/COUNT(id)), 2), 0) FROM coppeling.ag_calificaciones f WHERE f.id_alumno = mdl_user.id) ,' ')", 
		"mdl_user", 
		"Promedio General"));
	...
	array_push($columns_tmp, new Column(
		...
		"periodo_actual", 
		"periodo_actual", 
		"COALESCE((SELECT MAX(j.periodo) FROM coppeling.mdl_user_enrolments g INNER JOIN coppeling.mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = 18 INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = mdl_user.id), 0)", 
		"mdl_user", 
		"Periodo Actual"));
	...
	// 	"numero_telefonico",
	// 	"numero_telefonico",
	// 	"IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt inner join escolar.tb_alumnos ta on tnt.id_alumno=ta.id WHERE  tnt.eliminado=0 and ta.idmoodle = mdl_user.id and ta.id_plan_estudio=18  order by tnt.id DESC limit 1),'SIN NUMERO')", 
	// 	"mdl_user",
	// 	"Numero Telefonico")
	// );
	...
	array_push($columns_tmp, new Column(
		32, 
		1, 
		"correo_electronico_escolar", 
		"correo_electronico_escolar", 
		"IFNULL((SELECT 
					tb_personas.email 
				FROM 
					escolar.tb_personas 
					INNER JOIN escolar.tb_alumnos ON tb_alumnos.id_persona = tb_personas.id 
				WHERE 
					tb_alumnos.id_plan_estudio = 18 
					AND tb_alumnos.idmoodle = mdl_user.id 
				LIMIT 1) ,' ')", 
		"mdl_user", 
		"Correo Electronico Escolar"));
	...
	// 		"email", 
	// 		"email", 
	// 		"IFNULL((select IF(c.email is Null or c.email like '%agcollge%', null, c.email) from mdl_user a
	// 		 INNER JOIN escolar.tb_alumnos b on b.idmoodle = a.id and b.id_plan_estudio = $id_plan_estudio_columnas
	// 		 INNER JOIN escolar.tb_personas c on c.idmoodle = a.id limit 1),'Sin correo eléctronico')", 
	// 		"mdl_user", 
	// 		"Correo electrónico")
	// 	);
	...
	array_push($columns_tmp,new Column(
		...
		"email2",
		"IFNULL((SELECT
				IF(a.correo_electronico = '', 'NO DEFINIDO', a.correo_electronico) AS 'email2'
			FROM
				escolar.view_inscripciones a
			WHERE
				a.idmoodle = mdl_user.id
				AND a.id_plan_estudio = $id_plan_estudio_columnas
			LIMIT 1), 'NO DEFINIDO')", 
...
/*(SELECT
	...
	array_push($columns_tmp,new Column(
		...
		1,
		"generacion_inscripcion",
		"generacion_inscripcion",
		"IFNULL(
			(SELECT
				c.numero AS 'generacion_inscripcion'
			FROM
				escolar.tb_alumnos a
				INNER JOIN escolar.tb_alumnos_posibles_cargar b ON b.id_alumno = a.id
				LEFT JOIN escolar.tb_generaciones c ON c.id_plan_estudio = a.id_plan_estudio
					AND c.month_inscripcion = IFNULL(MONTH(a.periodo_inscripcion), MONTH(b.fecha_primera_carga))
					AND c.year_inscripcion = IFNULL(YEAR(a.periodo_inscripcion), YEAR(b.fecha_primera_carga))
			WHERE
				a.idmoodle = mdl_user.id
				AND a.id_plan_estudio = $id_plan_estudio_columnas
				AND a.prueba = 0
	...
	array_push($columns_tmp,new Column(
		...
		"IFNULL(
			...
			FROM
				...
				INNER JOIN escolar.tb_alumnos_posibles_cargar b ON b.id_alumno = a.id
				LEFT JOIN escolar.tb_generaciones c ON c.id_plan_estudio = a.id_plan_estudio
					AND c.month_inscripcion = MONTH(b.fecha_primera_carga)
					AND c.year_inscripcion = YEAR(b.fecha_primera_carga)
			WHERE
				a.idmoodle = mdl_user.id
				AND a.id_plan_estudio = $id_plan_estudio_columnas
				AND a.prueba = 0
	...
	array_push($columns_tmp,new Column(
		...
		"IFNULL(
			(SELECT
				c.numero AS 'generacion_periodo'
			FROM
				escolar.tb_alumnos a
				INNER JOIN escolar.tb_alumnos_posibles_cargar b ON b.id_alumno = a.id
				INNER JOIN escolar.tb_generaciones c ON c.id_plan_estudio = a.id_plan_estudio
					AND c.periodo_actual_cursamiento = b.periodo_cursando
			WHERE
				a.idmoodle = mdl_user.id
				AND a.id_plan_estudio = $id_plan_estudio_columnas
				AND a.prueba = 0
	...
	array_push($columns_tmp,new Column(
		...
		"IFNULL((SELECT
			...
			FROM
				escolar.tb_alumnos a
				INNER JOIN escolar.tb_personas b ON b.id = a.id_persona
				INNER JOIN escolar.tb_ciudades c ON c.id = b.ciudad
			WHERE
				a.idmoodle = mdl_user.id
				AND a.id_plan_estudio = $id_plan_estudio_columnas
			LIMIT 0,1), 'NO DEFINIDO')",
	...
	#a.promedio_general = (SELECT COALESCE(FORMAT((SUM(IF(f.calificacion > 10, (f.calificacion * 0.1), f.calificacion))/COUNT(id)), 2), 0) FROM ", @nombre_basedatos, ".ag_calificaciones f WHERE f.id_alumno = a.idmoodle),
	
	#a.periodo_actual = COALESCE((SELECT MAX(j.periodo) FROM ", @nombre_basedatos, ".mdl_user_enrolments g INNER JOIN ", @nombre_basedatos, ".mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = ", _idplanestudio, " INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = a.idmoodle), 0)
/*
id_alumno = moodle (ag_calificaciones)
id_materia = moodle (ag_calificaciones)
id_alumno = escolar
id_materia = escolar
idmoodle_alumno = moodle
idmoodle_mat5eria = moodle
*/
