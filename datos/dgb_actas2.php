...
if($generacion==1){
  ...
  $WHERE_ALUMNOS_G=$WHERE_ALUMNOS_G2;
            ...
            //ORDENAR POR MATRICULA O APELLIDO
             ...
             $query_alumnos="select ta.id_corporacion,ta.id_plan_estudio,ta.idmoodle,tp.curp, REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(tp.apellido1),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as apellido1,
  REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(tp.apellido2),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as apellido2,
   REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(tp.nombre),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as nombre,tba.* from tb_personas tp 
             ...
             WHERE   tba.generacion=".$generacion." order by tba.grupo ASC, tba.matricula ASC";
             $idx_alumno=0;
             $alumnos=array();
			...
			foreach ($alumnos as $al) {
				...
				foreach ($materias as $mat) {
                    ...
                    if((int)$al['id_corporacion']==2){
                        $query_calificaciones="SELECT count(ue.id) as cargada from prepacoppel.mdl_user_enrolments ue inner join prepacoppel.mdl_enrol en on en.id=ue.enrolid where ue.userid='".$al['idmoodle']."' and en.courseid='".$mat['idmoodle']."' and ue.status=1 ";
                       // $query_calificaciones="SELECT agc.calificacion from prepacoppel.ag_calificaciones agc where agc.id_materia='".$mat['idmoodle']."' AND agc.id_alumno='".$al['idmoodle']."' limit 1";
                        ...
                        if($result_cali=$mysql->Query($query_calificaciones)){
                            ...
                            $query_calificaciones="SELECT count(ue.id) as cargada from prepaley.mdl_user_enrolments ue inner join prepaley.mdl_enrol en on en.id=ue.enrolid where ue.userid='".$al['idmoodle']."' and en.courseid='".$mat['idmoodle_ley']."' and ue.status=1 ";
                        $cali=0;
                        if($result_cali=$mysql->Query($query_calificaciones)){
                            $reg = mysql_fetch_array($result_cali);
                            $cali=$reg['cargada'];
                            ...
                            $query_calificaciones="SELECT count(ue.id) as cargada from soriana.mdl_user_enrolments ue inner join soriana.mdl_enrol en on en.id=ue.enrolid where ue.userid='".$al['idmoodle']."' and en.courseid='".$mat['idmoodle_soriana']."' and ue.status=1 ";
                        $cali=0;
                        if($result_cali=$mysql->Query($query_calificaciones)){
                            $reg = mysql_fetch_array($result_cali);
                            $cali=$reg['cargada'];
                            ...
                            $query_calificaciones="SELECT count(ue.id) as cargada from `agcollege-ag`.mdl_user_enrolments ue inner join `agcollege-ag`.mdl_enrol en on en.id=ue.enrolid where ue.userid='".$al['idmoodle']."' and en.courseid='".$mat['idmoodle_agcollege']."' and ue.status=1 ";
                        $cali=0;
                        if($result_cali=$mysql->Query($query_calificaciones)){
                            $reg = mysql_fetch_array($result_cali);
                            $cali=$reg['cargada'];
				...
				/*	if($modo==1){ // GUARDA EN LA TABLA tb_dgb_actas
                    ...
                    if((int)$cali ==0 && (int)$al['id_corporacion'] != 3){ // si no la tiene terminada y noes soriana
                        ...
                        $actas[$idx]['matricula']=$al['matricula'];
                        $actas[$idx]['turno']=0;
...
//server PRUEBA online    2801    root    localhost   prepasumate Query   1   User lock   SELECT GET_LOCK('prepasumate-mdl_-session-441', 120)
             ...
             $table.="<thead>
                        <th style='border-style: solid;' >CORPORACION</th>
             			<th style='border-style: solid;' >Num.</th>
                        ...
                        <th style='border-style: solid;' >Curp</th>
             			<th style='border-style: solid;' >Matrícula</th>
                        <th style='border-style: solid;' >Clave de asignatura</th>
             			<th style='border-style: solid;' >Grado</th>
             			<th style='border-style: solid;' >Grupo</th>
             			<th style='border-style: solid;' >Turno</th>
             			<th style='border-style: solid;' >Tipo de inscripcion</th>
             		</thead>";
 				$table.="<tbody>";
             ...
             for ($i=0; $i < count($actas) ; $i++) { 
                ...
                $table.="<td style='border-style: solid;' >".$actas[$i]['matricula']."</td>";
                $table.="<td style='border-style: solid;' >".$actas[$i]['clave_asignatura']."</td>";
             	...
             	$table.="<td style='border-style: solid;' >".$actas[$i]['turno']."</td>";
             	$table.="<td style='border-style: solid;' >".$actas[$i]['tipo']."</td>";
