			...
			foreach ($alumnos as $al) {
				...
				foreach ($materias as $mat) {
                    ...
                    if((int)$al['id_corporacion']==2){ // coppel
                        ...
                        {
                            $query_calificaciones="SELECT agc.calificacion from nprepacoppel.ag_calificaciones agc where agc.id_materia='".$mat['idmoodle_prepacoppel_2020']."' AND agc.id_alumno='".$al['idmoodle']."' limit 1";
                            $cali=0;
                            if($result_cali=$mysql->Query($query_calificaciones)){
                                $reg = mysql_fetch_array($result_cali);
                                $cali=$reg['calificacion'];
