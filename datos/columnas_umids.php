<?php 
	...
	array_push($columns_tmp,new Column(1000,1,"responsable_admision","responsable_admision","IFNULL((select IF(escolar.tb_alumnos.id_usuario_responsable = 210, 'NO DEFINIDO', concat(usu.nombre,' ', ifnull(usu.apellidop,''),' ', ifnull(usu.apellidom,''))) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
				where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=59 limit 1),'')", "mdl_user","Responsable Admisión Inscripción") );	
				...
				WHERE escolar.tb_alumnos.idmoodle = mdl_user.id  AND escolar.tb_alumnos.id_plan_estudio = 59 LIMIT 1),'NO DEFINIDO')", "mdl_user","Responsable Gestión Escolar Inscripción") );
				//columna tipo 0  ()
	...
	array_push($columns_tmp,new Column(4,1,"tipo_status","tipo_status","IFNULL((select escolar.tb_alumnos_estados.descripcion from escolar.tb_personas 
		...
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=59 limit 1),' ')", "mdl_user","Descripcion status") );
	...
	array_push($columns_tmp,new Column(5,1,"suspended","Status","IFNULL((select escolar.tb_alumnos_estados.nombre_categoria from escolar.tb_personas 
		inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
		inner join escolar.tb_alumnos_estados on escolar.tb_alumnos_estados.id=escolar.tb_alumnos.estado
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=59 limit 1), 1)
	...
	array_push($columns_tmp,new Column(6,1,"n_empleado","n_empleado","IFNULL((select escolar.tb_alumnos.numero_empleado from escolar.tb_alumnos
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=59 limit 1),' ')", "mdl_user","#Empleado",true) );	
	...
	array_push($columns_tmp,new Column(14,1,"fecha_status","fecha_status","IFNULL((select escolar.tb_alumnos_estados_log.fecha from escolar.tb_personas 
		inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona and  escolar.tb_alumnos.id_plan_estudio = 59
		inner join escolar.tb_alumnos_estados_log on escolar.tb_alumnos_estados_log.id_alumno=escolar.tb_alumnos.id
		where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=59 order by escolar.tb_alumnos_estados_log.fecha DESC limit 1),' ')", "mdl_user","Fecha status",false) );
	...
	// array_push($columns_tmp,new Column(10,1,"telefonos","telefonos","(select group_concat(numero_telefonico,' - ') from escolar.tb_numeros_telefonicos inner join escolar.tb_alumnos on tb_alumnos.id = tb_numeros_telefonicos.id_alumno where tb_alumnos.id_plan_estudio = 59 and tb_alumnos.idmoodle = mdl_user.id group by tb_alumnos.id) ", "mdl_user","Teléfonos") );
	...
	$querisillo = "
		...
		FROM
			escolar.tb_alumnos
			INNER JOIN escolar.tb_personas ON tb_personas.id = tb_alumnos.id_persona AND tb_alumnos.id_plan_estudio = 59
	...
	$querisillo = "
		...
		WHERE
			...
			AND escolar.tb_alumnos.id_plan_estudio = 59
	...
	array_push($columns_tmp, new Column(27, 1, "periodo_inscripcion", "periodo_inscripcion", "IFNULL((select periodo_inscripcion from escolar.tb_alumnos where id_plan_estudio = 59 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Periodo Inscripcion"));

	array_push($columns_tmp,new Column(28, 1, "generacion_actual", "generacion_actual","IFNULL((SELECT id_generacion_academica from escolar.tb_alumnos where id_plan_estudio = 59 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user", "Generación Actual"));
	...
	array_push($columns_tmp, new Column(
		...
		"periodo_actual", 
		"periodo_actual", 
		"COALESCE((SELECT MAX(j.periodo) FROM coppeling.mdl_user_enrolments g INNER JOIN coppeling.mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = 59 INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = mdl_user.id), 0)", 
		"mdl_user", 
		"Periodo Actual"));
	...
	// 	"numero_telefonico",
	// 	"numero_telefonico",
	// 	"IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt inner join escolar.tb_alumnos ta on tnt.id_alumno=ta.id WHERE  tnt.eliminado=0 and ta.idmoodle = mdl_user.id and ta.id_plan_estudio=59  order by tnt.id DESC limit 1),'SIN NUMERO')", 
	// 	"mdl_user",
	// 	"Numero Telefonico")
	// );
...
/*(SELECT
					...
					WHERE
						...
						AND MONTH(b.fecha_primera_carga) = cc.month_inscripcion
						AND YEAR(b.fecha_primera_carga) = cc.year_inscripcion
					LIMIT 1)

					SE QUITA POR SOLICITUD Ticket #010882  #Generación periodo de inscripción*/
	array_push($columns_tmp,new Column(
		...
		"IFNULL(
			...
			FROM
				...
				LEFT JOIN escolar.tb_generaciones c ON c.id_plan_estudio = a.id_plan_estudio
					AND c.month_inscripcion = MONTH(a.periodo_inscripcion)
					AND c.year_inscripcion = YEAR(a.periodo_inscripcion)
			WHERE
				a.idmoodle = mdl_user.id
				AND a.id_plan_estudio = $id_plan_estudio_columnas
				AND a.prueba = 0
	...
	array_push($columns_tmp,new Column(
		...
		"IFNULL((SELECT
			...
			FROM
				escolar.tb_alumnos a
				INNER JOIN escolar.tb_alumnos_posibles_cargar b ON b.id_alumno = a.id AND a.id_plan_estudio = $id_plan_estudio_columnas
				INNER JOIN escolar.tb_generaciones c ON c.id_plan_estudio = a.id_plan_estudio AND a.prueba = 0
			WHERE
				a.idmoodle = mdl_user.id
