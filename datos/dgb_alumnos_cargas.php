...
if($generacion==1){
  ...
  $WHERE_ALUMNOS_G=$WHERE_ALUMNOS_G2;
            ...
            $query_alumnos="select tr.nomenclatura as region,ta.id_corporacion,ta.id_plan_estudio,ta.idmoodle,tp.curp, REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(tp.apellido1),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as apellido1,
  REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(tp.apellido2),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as apellido2,
   REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(tp.nombre),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as nombre,tba.* from tb_personas tp 
          ...
          $alumnos[$idx_alumno]['region']=$row_alumno['region'];
      ...
      // COPPEL AND md.periodo=".$grado." 

            $query_materias="

              SELECT  md.id as id_materia_dgb,tmi.id_moodle,tp.curp as curp_profesor,
   ...
   if((int)$al['id_corporacion']==2){ // coppel
                        ...
                        $query_calificaciones="SELECT agc.calificacion from `agcollege-ag`.ag_calificaciones agc where agc.id_materia='".$mat['idmoodle_ley']."' AND agc.id_alumno='".$al['idmoodle']."' limit 1";
                        $cali=0;
                        if($result_cali=$mysql->Query($query_calificaciones)){
                            $reg = mysql_fetch_array($result_cali);
                            $cali=$reg['calificacion'];
          ...
          $actas[$idx]['materias_alumno']=$mat_al;
...
//server PRUEBA online    2801    root    localhost   prepasumate Query   1   User lock   SELECT GET_LOCK('prepasumate-mdl_-session-441', 120)
             ...
             $table.="<thead>
                  ...
                  <th style='border-style: solid;' >Region</th>
                  <th style='border-style: solid;' >REALIZADAS</th>

                  <th style='border-style: solid;' >FALTANTES</th>

             		";

            foreach ($materias as $mat1) {
                $table.=" <th style='border-style: solid;' >".$mat1['nombre_dgb']."</th>";
            }
        ...
        $table.="</thead><tbody>";
             ...
             for ($i=0; $i < count($actas) ; $i++) { 
              ...
              $table.="<td style='border-style: solid;'>".$actas[$i]['corporacion']."</td>";
               ...
               $table.="<td style='border-style: solid;'>".$actas[$i]['region']."</td>";
           ...
           //  	$table.="<td style='border-style: solid;' >".$actas[$i]['folio']."</td>";
            // 	$table.="<td style='border-style: solid;' >".$actas[$i]['curp_profesor']."</td>";
               $table_materias="";
               $total_materias_faltantes=0;
               $total_materias_realizadas=0;
              foreach ($actas[$i]['materias_alumno'] as $mat2) {
                  $table_materias.=" <th style='border-style: solid;' >".$mat2['cali']."</th>";
                  if((int)$mat2['cali']<6){
                    $total_materias_faltantes++;
                  }else{
                    $total_materias_realizadas++;
                  }
              }

                $table.="<th style='border-style: solid;' >".$total_materias_realizadas."</th>";
               $table.="<th style='border-style: solid;' >".$total_materias_faltantes."</th>";
               $table.=$table_materias;
