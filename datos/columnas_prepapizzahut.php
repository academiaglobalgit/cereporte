<?php
		...
		$id_plan_estudio_columnas = 71;

	array_push($columns_tmp,new Column(3,1,"ingreso","ingreso","IFNULL((select ta.fecha_inscripcion 
		from escolar.tb_alumnos ta where ta.idmoodle=mdl_user.id AND ta.id_plan_estudio=71 limit 1),' ')", "mdl_user","Fecha Inscripcion") );
	...
	array_push($columns_tmp,new Column(130,1,"responsable_admision","responsable_admision","IFNULL((select IF(escolar.tb_alumnos.id_usuario_responsable = 210, 'NO DEFINIDO', concat(usu.nombre,' ', ifnull(usu.apellidop,''),' ', ifnull(usu.apellidom,''))) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
				where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=71 limit 1),'')", "mdl_user","Responsable Admisión Inscripción") );	
				...
				WHERE escolar.tb_alumnos.idmoodle = mdl_user.id  AND escolar.tb_alumnos.id_plan_estudio = 71 LIMIT 1),'NO DEFINIDO')", "mdl_user","Responsable Gestión Escolar Inscripción") );
...
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=71 limit 1),1)
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
inner join escolar.tb_alumnos_estados on escolar.tb_alumnos_estados.id=escolar.tb_alumnos.estado
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=71 limit 1),' ')", "mdl_user","Descripcion status") );
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=71 limit 1)=1,'Masculino','Femenino')
", "mdl_user","Sexo",true) );
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona and  escolar.tb_alumnos.id_plan_estudio = 71
...
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_corporacion=1 order by escolar.tb_alumnos_estados_log.fecha DESC limit 1),' ')", "mdl_user","Fecha status") );
	...
	*/
		...
		//columna tipo 1
	array_push($columns_tmp,new Column(6,1,"n_empleado","n_empleado","IFNULL((select escolar.tb_alumnos.numero_empleado from escolar.tb_alumnos
	where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=71 limit 1),' ')", "mdl_user","#Empleado",true) );	
...
/*
	...
	array_push($columns_tmp,new Column(88,1,"matricula","matricula_dgb","(
		SELECT tdb.matricula FROM escolar.tb_alumnos ta inner join escolar.tb_dgb_alumnos tdb on tdb.id_alumno=ta.id WHERE ta.idmoodle=mdl_user.id AND ta.id_plan_estudio=71 limit 1
...
*/
	...
	//	$columna= new Column(13,2,"data","materias_coppel","SELECT mc.IdMoodle as id,mc.Nombre as fullname ,mc.Periodo as periodo  FROM materias_cobaes mc  order by mc.Periodo ASC,mc.IdMatricula ASC", "mdl_user","Materias Cobaes");
		...
		array_push($columns_tmp,new Column(15,1,"numero_telefonico","numero_telefonico","IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt inner join escolar.tb_alumnos ta on tnt.id_alumno=ta.id WHERE  tnt.eliminado=0 and ta.idmoodle=mdl_user.id and ta.id_plan_estudio=71  order by tnt.id DESC limit 1),'SIN NUMERO')", "mdl_user","Numero Telefonico") );
		...
		$querisillo = "
			...
			FROM
				escolar.tb_alumnos
				INNER JOIN escolar.tb_personas ON tb_personas.id = tb_alumnos.id_persona AND tb_alumnos.id_plan_estudio = 71
		...
		$querisillo = "
			...
			WHERE
				...
				AND escolar.tb_alumnos.id_plan_estudio = 71
		...
		array_push($columns_tmp,new Column(998,1,"periodo_inscripcion","periodo_inscripcion","IFNULL((select periodo_inscripcion from escolar.tb_alumnos where id_plan_estudio = 71 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user","Periodo Inscripcion"));
		...
		$query = "
			...
			FROM
				escolar.tb_alumnos a
				INNER JOIN escolar.tb_peticiones b ON b.id_alumno = a.id AND a.id_plan_estudio = 71
			WHERE
				a.idmoodle = mdl_user.id
			ORDER BY 
		...
		array_push($columns_tmp, new Column(
			...
			"periodo_actual", 
			"periodo_actual", 
			"COALESCE((SELECT MAX(j.periodo) FROM prepaagcollege.mdl_user_enrolments g INNER JOIN prepaagcollege.mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = 71 INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = mdl_user.id), 0)", 
			"mdl_user", 
			"Periodo Actual")
		);
