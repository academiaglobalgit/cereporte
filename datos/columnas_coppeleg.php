<?php 
	...
	array_push($columns_tmp,new Column(3,1,"ingreso","ingreso","IFNULL((select ta.fecha_inscripcion 
		from escolar.tb_alumnos ta where ta.idmoodle=mdl_user.id AND ta.id_plan_estudio=62 limit 1),' ')", "mdl_user","Fecha Inscripcion") );
	...
	array_push($columns_tmp,new Column(130,1,"responsable_admision","responsable_admision","IFNULL((select IF(escolar.tb_alumnos.id_usuario_responsable = 210, 'NO DEFINIDO', concat(usu.nombre,' ', ifnull(usu.apellidop,''),' ', ifnull(usu.apellidom,''))) from escolar.tb_alumnos LEFT JOIN escolar.tb_usuarios usu ON usu.id = escolar.tb_alumnos.id_usuario_responsable
				where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=62 limit 1),'')", "mdl_user","Responsable Admisión Inscripción") );	
				...
				WHERE escolar.tb_alumnos.idmoodle = mdl_user.id  AND escolar.tb_alumnos.id_plan_estudio = 62 LIMIT 1),'NO DEFINIDO')", "mdl_user","Responsable Gestión Escolar Inscripción") );
...
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=62 limit 1),' ')", "mdl_user","Descripcion status") );
...
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=62 limit 1),' ')", "mdl_user","Motivo Baja") );
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona 
inner join escolar.tb_alumnos_estados on escolar.tb_alumnos_estados.id=escolar.tb_alumnos.estado
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=62 limit 1),1)
", "mdl_user","Categoria Status",false) );
	...
	where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=62 limit 1),' ')", "mdl_user","#Empleado",true) );	
...
inner join escolar.tb_alumnos on escolar.tb_personas.id=escolar.tb_alumnos.id_persona and  escolar.tb_alumnos.id_plan_estudio = 62
inner join escolar.tb_alumnos_estados_log on escolar.tb_alumnos_estados_log.id_alumno=escolar.tb_alumnos.id
where escolar.tb_alumnos.idmoodle=mdl_user.id AND escolar.tb_alumnos.id_plan_estudio=62 order by escolar.tb_alumnos_estados_log.fecha DESC limit 1),' ')", "mdl_user","Fecha status",false) );
	...
	array_push($columns_tmp,new Column(66,1,"data","tipo_empleado","(select mdl_user_info_data.data from mdl_user_info_data where mdl_user_info_data.userid = mdl_user.id and mdl_user_info_data.fieldid = 9 limit 1) ", "mdl_user","Tipo Empleado",false) );	
		...
		array_push($columns_tmp,new Column(10,1,"telefonos","telefonos","(select group_concat(numero_telefonico,' - ') from escolar.tb_numeros_telefonicos inner join escolar.tb_alumnos on tb_alumnos.id = tb_numeros_telefonicos.id_alumno where tb_alumnos.id_plan_estudio = 62 and tb_alumnos.idmoodle = mdl_user.id group by tb_alumnos.id) ", "mdl_user","Teléfonos") );
		...
		$querisillo = "
			...
			FROM
				escolar.tb_alumnos
				INNER JOIN escolar.tb_personas ON tb_personas.id = tb_alumnos.id_persona AND tb_alumnos.id_plan_estudio = 62
		...
		$querisillo = "
			...
			WHERE
				...
				AND escolar.tb_alumnos.id_plan_estudio = 62
		...
		array_push($columns_tmp,new Column(998,1,"periodo_inscripcion","periodo_inscripcion","IFNULL((select periodo_inscripcion from escolar.tb_alumnos where id_plan_estudio = 62 and tb_alumnos.idmoodle =  mdl_user.id LIMIT 1) ,' ')", "mdl_user","Periodo Inscripcion"));
		...
		$query = "
			...
			FROM
				escolar.tb_alumnos a
				INNER JOIN escolar.tb_peticiones b ON b.id_alumno = a.id AND a.id_plan_estudio = 62
			WHERE
				a.idmoodle = mdl_user.id
			ORDER BY 
	...
	array_push($columns_tmp, new Column(
		...
		"promedio_general", 
		"promedio_general", 
		"IFNULL((SELECT COALESCE(FORMAT((SUM(IF(f.calificacion > 10, (f.calificacion * 0.1), f.calificacion))/COUNT(id)), 2), 0) FROM nlic.ag_calificaciones f WHERE f.id_alumno = mdl_user.id) ,' ')", 
		"mdl_user",
		"Promedio General")
	);
	...
	array_push($columns_tmp, new Column(
		...
		"periodo_actual", 
		"periodo_actual", 
		"COALESCE((SELECT MAX(j.periodo) FROM nlic.mdl_user_enrolments g INNER JOIN nlic.mdl_enrol h ON h.id = g.enrolid INNER JOIN escolar.tb_materias_ids i ON i.id_moodle = h.courseid AND i.id_plan_estudio = 62 INNER JOIN escolar.tb_materias j ON j.id = i.id_materia WHERE g.userid = mdl_user.id), 0)", 
		"mdl_user", 
		"Periodo Actual")
	);
    ...
    array_push($columns_tmp,new Column(
        ...
        "numero_telefonico",
        "numero_telefonico",
        "IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt inner join escolar.tb_alumnos ta on tnt.id_alumno=ta.id WHERE  tnt.eliminado=0 and ta.idmoodle=mdl_user.id and ta.id_plan_estudio=62  order by tnt.id DESC limit 1),'SIN NUMERO')", 
        "mdl_user",
        "Numero Telefonico") );
