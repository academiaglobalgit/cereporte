<?php 
	class MateriasMoodle extends Connection {
	    ...
	    function GetEjerciciosByMateriaUnidad($bd="",$materia=0,$unidad=0){
	    	...
	    	*/
	        $query = "SELECT mp.id,mcm.course,mp.name,mp.content,mcs.section FROM ".$bd.".mdl_course_modules mcm 
	        inner join ".$bd.".mdl_page mp on mp.id=mcm.instance 
	        inner join ".$bd.".mdl_course_sections mcs on mcs.id = mcm.section
	        WHERE (mcs.section = 7 or mcs.section = 9) and mcm.indent=".$unidad." and mcm.module=15 and mcm.course=".$materia." ORDER BY mp.id ASC;";
	    ...
	    function GetEjerciciosHechos($bd="",$id_alumno=0,$materia=0,$unidad=0,$id_corporacion=0,$id_plan_estudio=0){
	    	...
	    	*/
	        ...
	        (
		        ...
		        inner join ".$bd.".mdl_page mp on mp.id=mcm.instance  
		         inner join ".$bd.".mdl_course_sections mcs on mcs.id = mcm.section
		        WHERE (mcs.section = 7 or mcs.section = 9) and mcm.indent=".$unidad." and mcm.module=15 and mcm.course=".$materia."
	        ) as totales, 
	    ...
	    function UpdateEjercicio($contenido='',$id_alumno=0,$materia=0,$id_ejercicio=0,$unidad=0,$id_corporacion=0,$id_plan_estudio=0,$estatus=0){
	       ...
	       $result=$this->Query($query);
			...
			if($result['success']){
				...
				if($estatus==1){
					$result['message']="Tu actividad integradora se ha finalizado correctamente.";

				}else{
					$result['message']="Tu progreso se ha guardado correctamente.";
