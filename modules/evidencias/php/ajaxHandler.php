	...
	if(isset($_POST['request'])){
		...
		switch ($request['request']) {
			...
			case "GetUCLEscuelas":
					...
					$result=$materiasmoodle->GetUCLEscuelas($res_plan['data'][0]['basededatos']);
			...
			case "GetEjerciciosByMateriaUnidad":
					...
					if(is_numeric($request['data']['course']) && is_numeric($request['data']['unidad'])){
						...
						$result['message'].=": ERROR: ".$res_plan['data'][0]['basededatos'];
					}else{
			...
			case "GetEjercicioAlumno":
					...
					if(is_numeric($request['data']['id_moodle']) && is_numeric($request['data']['course']) && is_numeric($request['data']['id_ejercicio']) && is_numeric($request['data']['unidad']) ){
						$result_materia=$materiasmoodle->GetMateriaEscolarByMoodle($request['data']['course'],$request['data']['id_plan_estudio']);
						$id_materia_escolar=$result_materia['data'][0]['id'];

						$result_usuario=$usuarios->GetAlumnoByMoodle($request['data']['id_moodle'],$request['data']['id_plan_estudio']);
						$id_alumno=$result_usuario['data'][0]['id'];
						
						$result=$materiasmoodle->GetEjercicioAlumno($id_alumno,$id_materia_escolar,$request['data']['id_ejercicio'],$request['data']['unidad'],$request['data']['id_corporacion'],$request['data']['id_plan_estudio']);
			...
			case "GetEjerciciosHechos":
					...
					if(is_numeric($request['data']['id_moodle']) && is_numeric($request['data']['course']) && is_numeric($request['data']['unidad']) )
					{
						...
						$result=$materiasmoodle->GetEjerciciosHechos($res_plan['data'][0]['basededatos'],$id_alumno,$request['data']['course'],$request['data']['unidad'],$request['data']['id_corporacion'],$request['data']['id_plan_estudio']);
					}else{
			...
			case "UpdateEjercicio":
					...
					if(isset($request['data']['contenido'])){
						...
						if( is_numeric($request['data']['id_moodle']) && is_numeric($request['data']['course']) && is_numeric($request['data']['id_ejercicio']) && is_numeric($request['data']['unidad']) && is_numeric($request['data']['id_corporacion'])  && is_numeric($request['data']['id_plan_estudio'])  ){
							...
							if(empty($id_alumno) || !is_numeric($id_alumno)){
								$result['message']="No se ha podido guardar, intente mas tarde. 004 ";

							}else if(empty($id_materia_escolar) || !is_numeric($id_materia_escolar)) {
								$result['message']="No se ha podido guardar, intente mas tarde. 005";

							}else{
								$result=$materiasmoodle->UpdateEjercicio($request['data']['contenido'],$id_alumno,$id_materia_escolar,$request['data']['id_ejercicio'],$request['data']['unidad'],$request['data']['id_corporacion'],$request['data']['id_plan_estudio'],$request['data']['estatus']);
							}
