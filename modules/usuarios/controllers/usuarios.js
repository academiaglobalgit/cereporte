$( document ).ready(function() {
	...
	$("#f_u_id_estado").change(function() { // cuando cambia el select del plan de estudio
		var idx=$("#f_i_id_region option:selected").attr('item-index');
		GetSucursales();
	...
	$("#f_u_id_sucursal").change(function() { // cuando cambia el select del plan de estudio
...
function GetRegionesUpdate(){
		...
		GetAjax(
			...
			"GetRegiones",
			function(response){
				RefreshComboBox("f_u_id_region",response,'id','nomenclatura');
...
function GetUCLEscuelas (){
		...
		GetAjax(
			...
			"GetUCLEscuelas",
			function(response){
				RefreshComboBox("f_i_id_ucl_escuela",response,'id','name');
			}
		);
}
...
/*######## usuariosS CLICKS LISTENERS ########*/
	...
	$("#btn_form_usuario_update").click(function(){
		SendAjax(
			...
			"UpdateUsuario",
			function(){
				$("#modal_usuario_update").modal('hide');
				GetUsuarios();
			});
	});
	...
	function RefreshUsuarios(usuarios_){
		...
		if(usuarios_.length==0 || usuarios_==null || usuarios_===undefined){
			...
			for (var i = 0; i < usuarios_.length; i++) {
	            ...
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['tel1']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['tel2']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['tel3']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['nombre_estado']+'</td>'; 

	            html_usuarios+='	<td >';
	            html_usuarios+='		<a href="#" class="update_usuarios" item-index="'+i+'"   > <span class="glyphicon glyphicon-cog" ></span></a>'; 
	            html_usuarios+='	</td>';
	            html_usuarios+='</tr>';
			}			
		}
		...
		$(".update_usuarios").click(function(){
			...
			$("#modal_title_usuario").empty().append('Editar Usuario: '+usuarios[idx_current_usuarios]['nombre']+' '+usuarios[idx_current_usuarios]['apellido1']+' '+usuarios[idx_current_usuarios]['apellido2']);
			...
			}else if(usuarios[idx_current_usuarios]['id_plan_estudio']==16){
