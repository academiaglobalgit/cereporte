<?php 
	class Usuarios extends Connection {
		...
		function GetAlumnosByMateria($bd='',$id_course=0,$id_corporacion=0,$id_plan_estudio=0,$filtro=''){

			$where="";
			if($filtro != '' ){
				$where=" WHERE (CONCAT(m.firstname,' ',m.lastname) LIKE '%".$filtro."%') OR (inf2.data LIKE '%".$filtro."%') ";
			}
			$materiasmoodle = new MateriasMoodle();
			$examenes=$materiasmoodle->GetExamenesParciales($bd,$id_course);
			$unidades=count($examenes['data']);
			

			$query="SELECT m.*,inf.data as escuela,inf2.data as numero_empleado,".$unidades." as unidades
				FROM ".$bd.".mdl_user_enrolments mue 
				INNER JOIN ".$bd.".mdl_enrol en on en.id=mue.enrolid and en.courseid=".$id_course."
				INNER JOIN ".$bd.".mdl_user m on mue.userid=m.id
				inner join ".$bd.".mdl_user_info_data inf2 on inf2.userid=m.id and inf2.fieldid=4
				left join ".$bd.".mdl_user_info_data inf on inf.userid=m.id and inf.fieldid=13
				".$where."
			ORDER BY m.id ASC;	
			" ;
			...
			if($result['success']){
				while($row=mysqli_fetch_assoc($result['data'])){
					...
					$array_escuela=explode(" ",$row['escuela']);
					$row['escuela']=$array_escuela[0];

					

					$result_usuario=$this->GetAlumnoByMoodle($row['id'],$id_plan_estudio);
					$id_alumno=$result_usuario['data'][0]['id'];

					$row['actividades_alumno']=array();

					for ($i=0; $i < $unidades; $i++) { 
						$actividades_alumno=$materiasmoodle->GetEjerciciosHechos($bd,$id_alumno,$id_course,$i,$id_corporacion,$id_plan_estudio);
						//por cada unidad es un indice totales y hechas

						if($actividades_alumno['data'][0]['totales']==null ){
							$actividades_alumno['data'][0]['totales']=0;
						}
						if($actividades_alumno['data'][0]['hechos']==null ){
							$actividades_alumno['data'][0]['hechos']=0;
						}
						$row['actividades_alumno'][$i]['hechos']=$actividades_alumno['data'][0]['hechos'];
						$row['actividades_alumno'][$i]['totales']=$actividades_alumno['data'][0]['totales'];
					}
		...
		function GetAlumnoByMoodle($id_moodle=0,$id_plan_estudio=0){
			$query="SELECT ta.*
			FROM escolar.tb_alumnos ta 
			where ta.idmoodle=".$id_moodle." and  ta.id_plan_estudio=".$id_plan_estudio."  order by ta.idmoodle   limit 1; " ;
		...
		function GetAlumnoMoodle($bd="",$id_moodle=0){
			$query="SELECT m.*
			FROM ".$bd.".mdl_user m 
			where m.id=".$id_moodle."   limit 1; " ;
		...
		function GetUsuarios($bd="",$id_plan_estudio=0,$id_moodle=0,$numero_empleado="null",$nombre=""){
			...
			$query="SELECT tp.*,ta.id as id_alumno,ta.idmoodle,ta.numero_empleado,ta.id_plan_estudio,m.username,concat(m.firstname,' ',m.lastname) as nombre_plataforma,ta.tipo, m.email,te.nombre as nombre_estado,tp.region ,
			IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt WHERE tnt.id_alumno=ta.id and tnt.eliminado=0  order by tnt.id DESC limit 1),'SIN NUMERO') as numero_telefonico
		...
		function InsertUsuario($bd="",$id_corporacion=0,$id_plan_estudio=0,$nombre="",$apellido1="",$apellido2="",$usuario="nodefinio",$numero_empleado=0,$fecha_nacimiento="0000-00-00", $telefono_casa="",$telefono_celular="",$telefono_alternativo="",$email="sincorreo@agcollege.edu.mx",$id_estado=0,$id_ciudad=0,$nombre_ciudad="no definido",$id_region=0,$nombre_region="no definido",$numero_sucursal=0,$nombre_sucursal="NO_DEFINIDO",$tipo_alumno=0,$sexo=0,$nomenclatura_ciudad="No Definido",$id_ucl_escuela=0,$documentacion_estatus=0,$curp=""){
				...
				if($result_infodata['success']){
					...
					$query_tb_personas="INSERT INTO escolar.tb_personas ( idmoodle, usuario, contrasena, permisos, nombre, apellido1, apellido2, sexo, fecha_nacimiento, rfc, ife, curp, estado_civil, colonia, calle, numero, codigopostal, ciudad, municipio, estado, region, sucursal, puesto_nombre, puesto, area, email, whatsapp, facebook, tel1, tel2, tel3, id_corporacion, corporacion_origen, prueba, activo, fecha_registro, actualizado, eliminado, fecha_eliminado) VALUES ( '".$new_id_moodle."', '".$usuario."', '".$contra."', '1', '".$nombre."', '".$apellido1."', '".$apellido2."', '".$sexo."', '".$fecha_nacimiento."', 'NORFC', 'NOIFE', ".$curp.", '1', '', '', '', '80000', '".$id_ciudad."', '1', '".$id_estado."','".$id_region."', '1', 'PUESTO NO DEFINIDO', '1', '0', '".$email."', '', '', '".$telefono_casa."', '".$telefono_celular."', '".$telefono_alternativo."', '".$id_corporacion."', '".$id_corporacion."', '0', '1', now(), now(), '0', '0000-00-00 00:00:00'); ";
						...
						if($result_tb_personas['success']){
							...
							$status=1;
							if((int)$id_plan_estudio==9 || (int)$id_plan_estudio==10){
							...
							$query_tb_alumnos="INSERT INTO escolar.tb_alumnos ( id_persona, idmoodle, asesor, numero_empleado, id_plan_estudio, estado,documentacion_estatus, fecha_reporte_estado, carga_automatica, acta_nacimiento, certificado_secundaria, certificado_parcial_estudios, tipo, prepa_equivalencia, id_rvoe, fecha_inscripcion, fecha_convocatoria, fecha_registro, actualizado, eliminado, fecha_eliminado, id_corporacion, curso_induccion, id_reponsable_curso_induccion) VALUES ( '".$new_id_persona."', '".$new_id_moodle."', '202', '".$numero_empleado."', '".$id_plan_estudio."', '".$status."','".$documentacion_estatus."', now(), '0', '0', '0', '0', '".$tipo_alumno."', '0', NULL, now(), '0000-00-00', now(),now(), NULL, '0000-00-00 00:00:00', '".$id_corporacion."', '0', NULL);";
