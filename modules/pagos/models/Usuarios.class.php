<?php 
	class Usuarios extends Connection {
		...
		function ReactivarAlumno($bd='',$id_moodle=0,$id_tb_alumno=0){
		 	
		 	$query_select="SELECT ta.estado,ta.id_plan_estudio,ta.documentacion_estatus FROM escolar.tb_alumnos ta WHERE ta.id=".$id_tb_alumno." ";
		 	$result_select=$this->Query($query_select);
		 	$result=$result_select;
		 	if($result_select['success']){
		 		$estatus=0;
		 		$id_plan_estudio=0;
		 		$documentacion_estatus=0;

		 		while($row_select=mysqli_fetch_assoc($result_select['data'])){
		 			$estatus=$row_select['estado'];
		 			$id_plan_estudio=$row_select['id_plan_estudio'];
		 			$documentacion_estatus=$row_select['documentacion_estatus'];
		 		}

		 		if((int)$estatus == 9){
		 					 		$new_estatus=0;
				 		$new_suspended=0;
				 		if($id_plan_estudio==9 or $id_plan_estudio==10 or $id_plan_estudio==29 or $id_plan_estudio==30){ //si es prepatoks o lic toks
				 			if($documentacion_estatus==1){
				 				$new_estatus=13;
				 			}else if($documentacion_estatus==2) {
				 				$new_estatus=14;
				 			}else if($documentacion_estatus==3){
				 				$new_estatus=15;
				 			}else{
				 				$new_estatus=13;
				 			}
				 		...
				 		$query_update_alumno="UPDATE escolar.tb_alumnos ta SET ta.estado=".$new_estatus.",ta.suspencion_pago=0 WHERE ta.id=".$id_tb_alumno." limit 1";
				 		$result_update_alumno=$this->Query($query_update_alumno);

				 		if($result_update_alumno['success']){

				 			$query_plataforma="UPDATE ".$bd.".mdl_user m SET m.suspended=".$new_suspended." WHERE m.id=".$id_moodle." LIMIT 1;";

				 			$result_update_plataforma=$this->Query($query_plataforma);

				 			if($result_update_plataforma['success']){

				 				$result['message']="Se Reactivó correctamente el alumno.";
				 				$this->Bitacora($_SESSION['id_persona'],"REACTIVACION","MODIFICACION",$bd.": Alumno REACTIVADO con ID MOODLE: ".$id_moodle." , el ID en la tabla TB_ALUMNOS: ".$id_tb_alumno." ");

				 			}else{
				 				$result=$result_update_plataforma;
				 				//$result['message']="No se ha podido reactivar al alumno, intente mas tarde. code: 004";
				 			}


				 		}else{
				 			$result=$result_update_alumno;
				 			$result['message']="No se ha podido reactivar al alumno, intente mas tarde. code: 002";
				 		}

		 		} else{
		 			$result['data']=[];
		 			$result['success']=false;
		 			$result['message']="No se ha podido reactivar al alumno, solo se pueden activaar alumnos suspendidos por falta de pago.";
		 		...
		 		$result=$result_select;
		 		$result['message']="No se ha podido Reactivar al alumno. intente mas tarde. code: 001";
		 	}
		...
		function SuspenderPorPagoAlumno($bd='',$id_moodle=0,$id_tb_alumno=0){
		 	

		 		$new_estatus=9;
		 		$new_suspended=2;

		 		$query_update_alumno="UPDATE escolar.tb_alumnos ta SET ta.estado=".$new_estatus.",ta.suspencion_pago=1 WHERE ta.id=".$id_tb_alumno." limit 1";
		 		$result_update_alumno=$this->Query($query_update_alumno);
		 		$result=$result_update_alumno;
		 		if($result_update_alumno['success']){
		 			...
		 			if($result_update_plataforma['success']){
		 				...
		 				$result['message']="Se Suspendió correctamente el alumno.";
		 				$this->Bitacora($_SESSION['id_persona'],"SUSPENCION","MODIFICACION",$bd.": Alumno SUSPENDIDO POR PAGO con ID MOODLE: ".$id_moodle." , el ID en la tabla TB_ALUMNOS: ".$id_tb_alumno." ");

		 			}else{
		 				$result=$result_update_plataforma;
		 				$result['message']="No se ha podido suspender al alumno, intente mas tarde. code: 004";
		 			}


		 		}else{
		 			$result=$result_update_alumno;
		 			$result['message']="No se ha podido suspender al alumno, intente mas tarde. code: 002";
		 		}
		...
		function GetUsuarios($id_moodle=0,$numero_empleado="null",$nombre="",$numero_referencia="null"){
			$where="";
			...
			if($numero_referencia==0 || $numero_referencia=="" || $numero_referencia=="null"){

			}else{
				$where.=" or ta.numero_referencia LIKE '%".$numero_referencia."%' ";
			}
			...
			if($id_moodle==0 || $id_moodle=="" || $id_moodle=="null"){
				...
				$where.=" or ta.idmoodle = '".$id_moodle."' ";
			}

			$query="SELECT tp.*,
			tae.descripcion as estatus_desc,
			tae.label as estatus_label,
			ta.id as id_alumno,
			ta.idmoodle,
			ta.numero_empleado,
			ta.id_plan_estudio,
			tp.usuario as username,
			concat(tp.nombre,' ',tp.apellido1,' ',tp.apellido2) as nombre_completo,ta.tipo, tp.email,te.nombre as nombre_estado,tp.region ,ta.numero_referencia,tpe.nombre_corto as plataforma,
			IFNULL((SELECT tnt.numero_telefonico FROM escolar.tb_numeros_telefonicos tnt WHERE tnt.id_alumno=ta.id and tnt.eliminado=0  order by tnt.id DESC limit 1),'SIN NUMERO') as numero_telefonico,tp.area as id_area
			FROM escolar.tb_personas tp 
			inner join escolar.tb_alumnos ta on ta.id_persona=tp.id 
			inner join escolar.tb_plan_estudio tpe on tpe.id=ta.id_plan_estudio
			inner join escolar.tb_alumnos_estados tae on tae.id=ta.estado
			left join escolar.tb_estados te on te.id=tp.estado

			where ( (1=2) ".$where." )  order by tpe.nombre_corto ASC  limit 50; " ;
		...
		function UpdateUsuario($bd="",$id_corporacion=0,$id_plan_estudio=0,$id_moodle=0,$id_persona=0,$nombre="",$apellido1="",$apellido2="",$usuario="nodefinio",$numero_empleado,$fecha_nacimiento="0000-00-00",$tipo_alumno=1,$numero_sucursal='',$nombre_sucursal='',$horario='',$id_estado=1,$sexo=0,$email='noemail@agcollege.edu.mx',$id_sucursal=0,$id_region=1,$nombre_region='no definido',$curp="",$id_area=12,$tipo_usuario="Alumno"){
			...
			if($id_plan_estudio==9 OR $id_plan_estudio==10 OR $id_plan_estudio==29 OR $id_plan_estudio==30){ // si es toks
			...
			$query_tb_personas="UPDATE escolar.tb_personas tp SET tp.curp=".$curp.",tp.sexo='".$sexo."',tp.fecha_nacimiento='".$fecha_nacimiento."', tp.contrasena='".$contra."', tp.usuario='".$usuario."', tp.nombre='".$nombre."', tp.apellido1='".$apellido1."', tp.apellido2='".$apellido2."',tp.sucursal='".$id_sucursal."',tp.estado='".$id_estado."',tp.region='".$id_region."',tp.area='".$id_area."' WHERE tp.id='".$id_persona."' limit 1 " ;
			...
			if($result_tb_personas['success'] && $result_mdl_user['success'] && $result_tb_alumnos['success'] && $result_infodata_n_empleado['success'] && $result_infodata_tipo_alumno['success']){
				...
				$semigro=false;
				if((int)$id_plan_estudio==4){
					$semigro=$migrapersonas_produccion->MigrarByPersonaV2($bd,$id_corporacion,$id_plan_estudio,10,'Alumno',$id_moodle,$id_persona,false); 
				}else{
					$semigro=$migrapersonas_produccion->MigrarTelefono($bd,$id_corporacion,$id_plan_estudio,1,'Alumno',$id_moodle,$id_persona,false); 
				}
				$pg->Close();
				if($semigro){
					$result['message'].=".";
		...
		function InsertUsuario($bd="",$id_corporacion=0,$id_plan_estudio=0,$nombre="",$apellido1="",$apellido2="",$usuario="nodefinio",$numero_empleado=0,$fecha_nacimiento="0000-00-00", $telefono_casa="",$telefono_celular="",$telefono_alternativo="",$email="sincorreo@agcollege.edu.mx",$id_estado=0,$id_ciudad=0,$nombre_ciudad="no definido",$id_region=1,$nombre_region="no definido",$numero_sucursal=0,$nombre_sucursal="NO_DEFINIDO",$tipo_alumno=0,$sexo=0,$nomenclatura_ciudad="No Definido",$id_escuela=0,$documentacion_estatus=0,$curp="",$id_area=12,$tipo_usuario="Alumno",$generacion=0){
			...
			if($tipo_usuario != "Alumno"){
				$contra=md5("12345".$salt_hash); // genera la contraseña si es usuario de prueba
				...
				if((int)$id_plan_estudio==4){ //es ley
							...
							INSERT INTO ".$bd.".mdl_user_info_data ( userid, fieldid, data, dataformat) VALUES 
							  ( '".$new_id_moodle."', '4', '".$numero_empleado."', '0'),
								...
								( '".$new_id_moodle."', '10', '".$tipo_usuario."', '0'),
							...
							if((int)$id_plan_estudio==2){ //inserta en prepacoppel.ag_alumnos para el sistema viejo de casa
								$query_ag_alumnos=" INSERT INTO ".$bd.".ag_alumnos (id,numero_trabajador) values ('".$new_id_moodle."','".$numero_empleado."') ";
								$result_ag_alumnos=$this->Query($query_ag_alumnos);
							}


							if((int)$id_plan_estudio==16 || (int)$id_plan_estudio==21 || (int)$id_plan_estudio==19){ // SI ES  ESCUELAS



										if((int)$id_plan_estudio==16){
															$query_infodata=" 
												INSERT INTO ".$bd.".mdl_user_info_data ( userid, fieldid, data, dataformat) VALUES 
												( '".$new_id_moodle."', '1', '".$tipo_usuario."', '0'),
												 ( '".$new_id_moodle."', '3', 'No Definido', '0'),
												  ( '".$new_id_moodle."', '4', '".$numero_empleado."', '0'),
												   ( '".$new_id_moodle."', '5', '".$numero_sucursal."', '0'),
													...
													( '".$new_id_moodle."', '12', '".$generacion."', '0') ,
													( '".$new_id_moodle."', '13', '".$id_escuela."', '0')  ";
													} else{
															$query_infodata=" 
												INSERT INTO ".$bd.".mdl_user_info_data ( userid, fieldid, data, dataformat) VALUES 
												( '".$new_id_moodle."', '1', '".$tipo_usuario."', '0'),
				...
				if($result_infodata['success']){
					...
					$query_tb_personas="INSERT INTO escolar.tb_personas ( idmoodle, usuario, contrasena, permisos, nombre, apellido1, apellido2, sexo, fecha_nacimiento, rfc, ife, curp, estado_civil, colonia, calle, numero, codigopostal, ciudad, municipio, estado, region, sucursal, puesto_nombre, puesto, area, email, whatsapp, facebook, tel1, tel2, tel3, id_corporacion, corporacion_origen, prueba, activo, fecha_registro, actualizado, eliminado, fecha_eliminado) VALUES ( '".$new_id_moodle."', '".$usuario."', '".$contra."', '1', '".$nombre."', '".$apellido1."', '".$apellido2."', '".$sexo."', '".$fecha_nacimiento."', 'NORFC', 'NOIFE', ".$curp.", '1', '', '', '', '80000', '".$id_ciudad."', '1', '".$id_estado."','".$id_region."', '1', 'PUESTO NO DEFINIDO', '1', '".$id_area."', '".$email."', '', '', '".$telefono_casa."', '".$telefono_celular."', '".$telefono_alternativo."', '".$id_corporacion."', '".$id_corporacion."', '0', '1', now(), now(), '0', '0000-00-00 00:00:00'); ";
						...
						if($result_tb_personas['success']){
							...
							$status=1;
							if((int)$id_plan_estudio==9 OR (int)$id_plan_estudio==10 OR (int)$id_plan_estudio==29 OR (int)$id_plan_estudio==30){
								if((int)$documentacion_estatus==1){
									$status=13;
								}else if((int)$documentacion_estatus==2){
									$status=14;
								}else if((int)$documentacion_estatus==3){
									$status=15;
								}
							}
							...
							$query_tb_alumnos="INSERT INTO escolar.tb_alumnos ( id_persona, idmoodle, asesor, numero_empleado, id_plan_estudio, estado,documentacion_estatus, fecha_reporte_estado, carga_automatica, acta_nacimiento, certificado_secundaria, certificado_parcial_estudios, tipo, prepa_equivalencia, id_rvoe, fecha_inscripcion, fecha_convocatoria, fecha_registro, actualizado, eliminado, fecha_eliminado, id_corporacion, curso_induccion, id_reponsable_curso_induccion,prueba) VALUES ( '".$new_id_persona."', '".$new_id_moodle."', '202', '".$numero_empleado."', '".$id_plan_estudio."', '".$status."','".$documentacion_estatus."', now(), '0', '0', '0', '0', '".$tipo_alumno."', '0', NULL, now(), '0000-00-00', now(),now(), NULL, '0000-00-00 00:00:00', '".$id_corporacion."', '0', NULL,".$prueba."); ";
							...
							if($result_tb_alumnos['success']){ //registro èxitoso
								...
								if(!empty($telefono_celular) && $telefono_celular != "" ){
									...
									/*INSERTA EN JL TELEFONOS SOLO EN PREPACOPPEL*/
									if($id_plan_estudio==2){
										$query_tb_numeros2_coppel="INSERT INTO ".$bd.".jl_telefonos ( userid, telefono, status, usuario_alta, fecha_alta) VALUES ( '".$new_id_moodle."', '".$telefono_celular."', 'A', '1787',now())";
										$result_tb_numeros2_coppel=$this->Query($query_tb_numeros2_coppel);
									}
								...
								$result['message']=" El Alumno ".$nombre." ".$apellido1." ".$apellido2." se ha Inscrito correctamente a la plataforma.  \n usuario: ".$usuario." contraseña: ".$numero_empleado." ";
								...
								$migrapersonas_produccion= new MigraPersonas($this, $pg); 
								if((int)$id_plan_estudio==4){
									$migrapersonas_produccion->MigrarByPersonaV2($bd,$id_corporacion,$id_plan_estudio,10,'Alumno',$new_id_moodle,0,false); 
								}else{
									$migrapersonas_produccion->MigrarByPersonaV2($bd,$id_corporacion,$id_plan_estudio,1,'Alumno',$new_id_moodle,0,false); 
								}

								$pg->Close();

							}else{
								$result['message']="Lo sentimos el usuario no se ha podido registrar, intente mas tarde. 004".$result_tb_alumnos['message'];
								$this->rollback();
							}
