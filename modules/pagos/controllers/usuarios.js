$( document ).ready(function() {
	...
	function GetUsuarios(){
			var data_ajax= {
				...
				'numero_referencia':$("#numero_referencia").val(),
...
function GetEscuelas (){
	...
	function GetMotivosBajasTelefonos (){
		...
		function GetServicios (plan_estudios){
			var data_ajax= {
				id_plan_estudio:plan_estudios
				};
			...
			GetAjax(
				...
				"GetServicios",
				function(response){
					RefreshComboBox('form_pago_nomenclatura',response,'id','nomenclatura_nombre');
				}
			);
	}


		function GetPagosByAlumno (id){
			var data_ajax= {
				id_alumno:id
				};
...
/*######## usuarios pagos CLICKS LISTENERS ########*/
	...
	$("#btn_insert_pago").click(function(){
		SendAjax(
			"form_pago",
			"InsertPago",
			function(){
	...
	$("#btn_suspender").click(function(){
		SendAjax(
			"form_reactivar",
			"SuspenderPorPagoAlumno",
			function(){
				GetUsuarios();
			});
	});
	...
	function RefreshComboBoxRegion(id_select,arrayData,keyArray_value,keyArray_option){
		...
		if(arrayData.length==0 || arrayData==null || arrayData===undefined){
			...
			regiones=arrayData;
			for (var i = 0; i < arrayData.length; i++) {
	            html_combobox+='	<option  value="'+arrayData[i][keyArray_value]+'"  > '+arrayData[i][keyArray_option]+' - '+arrayData[i]['nombre']+'</option>'; 
			}		
			$("#"+id_select).empty().append(html_combobox);
		}
	}
	...
	function RefreshRegiones(regiones_){
		...
		if(regiones_.length==0 || regiones_==null || regiones_===undefined){
			...
			for (var i = 0; i < regiones_.length; i++) {
	            html_regiones+='	<option  item-index="'+i+'" value="'+regiones_[i]['id']+'"  >'+regiones_[i]['nombre']+'</option>'; 
	...
	function RefreshPagosAlumnos(pagos_){
		var html_pagos="";
		if(pagos_.length==0 || pagos_==null || pagos_===undefined){
			html_pagos+='<tr  class="text-center" >';
            html_pagos+='	<td colspan="11" >No se han registrado pagos para este alumno.</td>'; 
            html_pagos+='</tr>';
            pagos=[];
		}else{
			pagos=pagos_;
			for (var i = 0; i < pagos_.length; i++) {
				html_pagos+='<tr>';
				html_pagos+='	<td   class="active" item-index="'+i+'"  >'+pagos_[i]['id']+'</td>'; 
	            html_pagos+='	<td   item-index="'+i+'"  >'+pagos_[i]['tipo']+'</td>'; 
	            html_pagos+='	<td   item-index="'+i+'"  >'+pagos_[i]['servicio']+'</label></td>'; 
	            html_pagos+='	<td   class="active item-index="'+i+'"  >$'+pagos_[i]['monto']+'</td>'; 
	            html_pagos+='	<td   item-index="'+i+'"  >'+pagos_[i]['fecha_pago']+'</td>'; 
	            html_pagos+='	<td   item-index="'+i+'"  >'+pagos_[i]['fecha_periodo']+'</td>'; 
	            html_pagos+='	<td   item-index="'+i+'"  >'+pagos_[i]['fecha_registro']+'</td>'; 
	            html_pagos+='	<td >';
	            html_pagos+='	</td>';
	          	html_pagos+='	<td >';
	            html_pagos+='	</td>';
		...
		$("#content_pagos").empty().append(html_pagos);
	}
	function RefreshUsuarios(usuarios_){
		...
		if(usuarios_.length==0 || usuarios_==null || usuarios_===undefined){
            ...
            html_usuarios+='	<td colspan="11" >No se encontraron alunos con esos datos o no se encuentra registrado en la base de datos.</td>'; 
            html_usuarios+='</tr>';
            usuarios=[];
		}else{
			usuarios=usuarios_;
			for (var i = 0; i < usuarios_.length; i++) {
				html_usuarios+='<tr>';
				html_usuarios+='	<td   class="active" item-index="'+i+'"  >'+usuarios_[i]['plataforma']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['idmoodle']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  ><label class="label label-'+usuarios_[i]['estatus_label']+'" >'+usuarios_[i]['estatus_desc']+'</label></td>'; 

	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['numero_empleado']+'</td>'; 
	            html_usuarios+='	<td  class="active" item-index="'+i+'"  >'+usuarios_[i]['nombre_completo']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['numero_telefonico']+'</td>'; 
	            html_usuarios+='	<td   item-index="'+i+'"  >'+usuarios_[i]['nombre_estado']+'</td>'; 
	            html_usuarios+='	<td >';
	           	html_usuarios+='		<button type="button" class="btn btn-default pagos_lista" item-index="'+i+'"   > <i class="glyphicon glyphicon-list"></i></button>'; 
	            html_usuarios+='	</td>';
		...
		$(".pagos_lista").click(function(){
			var index=parseInt($(this).attr("item-index"));
			idx_current_usuarios=index;
			$("#view_usuarios").hide();
			$("#view_pagos_usuarios").show();
			GetPagosByAlumno(usuarios[idx_current_usuarios]['id_alumno']);
		...
		$(".pagos_usuarios").click(function(){
			...
			if(usuarios[idx_current_usuarios]['id_plan_estudio']!=9 &&  usuarios[idx_current_usuarios]['id_plan_estudio'] != 10 ){
				GetServicios(9); // saca los servicios de prepatoks para las otras plataformas en caso de no ser toks
			}else{
				GetServicios(usuarios[idx_current_usuarios]['id_plan_estudio']);
			}
			...
			$("#form_reactivar_id_moodle").val(usuarios[idx_current_usuarios]['idmoodle']);
			$("#form_reactivar_id_persona").val(usuarios[idx_current_usuarios]['id']);
			$("#form_reactivar_id_plan_estudio").val(usuarios[idx_current_usuarios]['id_plan_estudio']);
			$("#form_reactivar_id_alumno").val(usuarios[idx_current_usuarios]['id_alumno']);
