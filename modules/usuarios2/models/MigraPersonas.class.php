				...
				if($mysql_result_escolar['success']){
					while ($row=mysqli_fetch_assoc($mysql_result_escolar['data'])) {
						...
						if($lastid==0 || !is_numeric($lastid)){ // si el id ya no existe en la nueva BASE DE DATOS masterag lo inserta
							...
							//QUERY DE INSERT EN masterag.tb_personas
							$pg_query='INSERT INTO "masterag"."tb_personas" ( "id_rol", "nombre1", "nombre2", "apellido1", "apellido2", "fecha_nacimiento", "sexo", "curp", "id_estado_civil", "fecha_registro", "fecha_modificacion","eliminado") VALUES ('.$rol.',\''.$primer_nombre.'\',\''.$segundo_nombre.'\',\''.$row['apellido1'].'\',\''.$row['apellido2'].'\',\''.$row['fecha_nacimiento'].'\',\''.$row['sexo'].'\',\''.$row['curp'].'\','.$estadocivil.',\''.$row['fecha_registro'].'\',now(),'.$row['eliminado'].');';
							...
							if($pg_result['success']){
								...
								if(!$pg_result_ids['success']){
									...
									if($interno){
										...
										$this->MigrarUsuarios($lastid,$row['idmoodle'],$row['id'],$row['id_alumno'],$bd); //migrar tb_usuarios
										$this->MigrarPersonasInscripcion($lastid,$row['idmoodle'],$row['id'],$row['id_alumno']); //migrar tb_personas_inscripcion
										$this->MigrarPersonasTelefonosV2($lastid,$row['idmoodle'],$row['id'],$row['id_alumno'],$bd); //migrar tb_personas_telefonos
							...
							if($pg_result['success']){
								...
								if($interno){
									...
									$telefonos=$this->MigrarPersonasTelefonosV2($lastid,$row['idmoodle'],$row['id'],$row['id_alumno'],$bd); //migrar tb_personas_telefonos
									

									if($perfil && $empresa && $usuario && $inscripcion && $telefonos){
										//UPDATE SUCCESS
										$updates++;
									}else{
...
/*
	...
	function MigrarPersonasTelefonosV2($id_nuevo,$id_moodle,$id_persona,$id_alumno,$bd)
	{
			//TB NUMEROS TELEFONICOS
			$mysql_select_tb_numero_tel='SELECT tp.*,tu.id_persona as asesor FROM escolar.tb_numeros_telefonicos tp 
			inner join escolar.tb_usuarios tu on tu.id=tp.id_asesor where tp.id_alumno='.$id_alumno.' ';
			$mysql_result_tb_numero_tel=$this->mysql->Query($mysql_select_tb_numero_tel);
			...
			if($mysql_result_tb_numero_tel['success']){
				...
				while ($row = mysqli_fetch_assoc($mysql_result_tb_numero_tel['data'])) {
					...
					$existe_id=0;
					$telefono_nuevo=null;
					$pg_select_query='SELECT * FROM "masterag"."tb_personas_telefonos" WHERE  "masterag"."tb_personas_telefonos"."id_viejo"='.$row['id'].' LIMIT 1';
					$pg_result_select=$this->pg->Query($pg_select_query);
					while ($line = pg_fetch_array($pg_result_select['data'], null, PGSQL_ASSOC)) {
					   $telefono_nuevo=$line;
					   $existe_id=$telefono_nuevo['id'];
					}
					
					
					$pg_select_query_asesor='SELECT "id_nuevo" FROM "masterag"."tb_ids_escolar" WHERE "tb_ids_escolar"."id_persona"='.$row['asesor'].' LIMIT 1';
					$pg_result_select_asesor=$this->pg->Query($pg_select_query_asesor);
					while ($line = pg_fetch_array($pg_result_select_asesor['data'], null, PGSQL_ASSOC)) {
					    foreach ($line as $col_value) {
					        $id_persona_asesor=$col_value;
					...
					if(is_numeric($existe_id) &&  $existe_id > 0 ){

						$pg_query_update='UPDATE "masterag"."tb_personas_telefonos" SET "numero"=\''.$row['numero_telefonico'].'\',"fecha_modificacion"=now(),"id_persona_registra"='.$id_persona_asesor.',"eliminado"='.$row['eliminado'].',"id_motivo_baja"='.$row['id_motivo_baja'].' WHERE "id"='.$existe_id.' ';

						$pg_result_update=$this->pg->Query($pg_query_update);

						if(!$pg_result_update['success'])
						{
						...
						$pg_query_ins='INSERT INTO "masterag"."tb_personas_telefonos" ("id_persona", "numero","fecha_registro" ,"id_persona_registra","eliminado","id_motivo_baja","id_viejo") VALUES ( '.$id_nuevo.',\''. $row['numero_telefonico'].'\', now(),'.$id_persona_asesor.','.$row['eliminado'].','.$row['id_motivo_baja'].','.$row['id'].');';
	...
	function MigrarTelefono($id_telefono)
	{
			//TB NUMEROS TELEFONICOS
			$mysql_select_tb_numero_tel='SELECT tp.*,tu.id_persona as asesor,ta.id_persona FROM escolar.tb_numeros_telefonicos tp 
			inner join escolar.tb_usuarios tu on tu.id=tp.id_asesor
			inner join escolar.tb_alumnos ta on ta.id=tp.id_alumno
			 where tp.id='.$id_telefono.' ';
			$mysql_result_tb_numero_tel=$this->mysql->Query($mysql_select_tb_numero_tel);
				...
				while ($row = mysqli_fetch_assoc($mysql_result_tb_numero_tel['data'])) {
					...
					while ($line = pg_fetch_array($pg_result_select['data'], null, PGSQL_ASSOC)) {
					   ...
					   $existe_id=$telefono_nuevo['id'];
						...
						$pg_select_query_persona_nueva='SELECT "id_nuevo" FROM "masterag"."tb_ids_escolar" WHERE "tb_ids_escolar"."id_persona"='.$row['id_persona'].' LIMIT 1';
					$pg_result_select_persona_nueva=$this->pg->Query($pg_select_query_persona_nueva);
					while ($line = pg_fetch_array($pg_result_select_persona_nueva['data'], null, PGSQL_ASSOC)) {
					    foreach ($line as $col_value) {
					        $id_persona_nueva=$col_value;
					...
					if(is_numeric($existe_id) &&  $existe_id > 0 ){
						...
						$pg_query_ins='INSERT INTO "masterag"."tb_personas_telefonos" ("id_persona", "numero","fecha_registro" ,"id_persona_registra","eliminado","id_motivo_baja","id_viejo") VALUES ( '.$id_persona_nueva.',\''. $row['numero_telefonico'].'\', now(),'.$id_persona_asesor.','.$row['eliminado'].','.$row['id_motivo_baja'].','.$row['id'].');';
