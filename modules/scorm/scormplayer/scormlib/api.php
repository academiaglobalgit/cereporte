...
alert(LMSGetValue("cmi.objectives._count"));
// ------------------------------------------
//   Status Flags
// ------------------------------------------
...
function LMSCommit(dummyString) {
    ...
    var params = 'scormid=<?php print $scormid; ?>&code='+d.getTime();
...
/*
    params += "&data[cmi.interactions.n.id]="+urlencode(cache['cmi.interactions.n.id']);
    params += "&data[cmi.interactions.n.time]="+urlencode(cache['cmi.interactions.n.time']);
    params += "&data[cmi.interactions.n.result]="+urlencode(cache['cmi.interactions.n.result']);
    params += "&data[cmi.interactions.n.latency]="+urlencode(cache['cmi.interactions.n.latency']);
    params += "&data[cmi.interactions.n.weighting]="+urlencode(cache['cmi.interactions.n.weighting']);
    params += "&data[cmi.interactions.n.objectives._count]="+urlencode(cache['cmi.interactions.n.objectives._count']);
*/


/*
    for (var i = 0; i < 1; i++) { // recorre todas las preguntas del examen deacuerdo al Score
        params += "&data[cmi.interactions."+i+".id]="+urlencode(cache['cmi.interactions.'+i]);
        params += "&data[cmi.interactions.n.correct_responses."+i+"]="+urlencode(cache['cmi.interactions.n.correct_responses.'+i]);
    }
*/
...
function LMSFinish(dummyString) {
	...
	req.open('GET','finish.php?scormid=<?php print $scormid; ?>&code='+d.getTime(),false);
