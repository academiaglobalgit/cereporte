<?php 	
	...
	function GetEscuelaAlumno($id_moodle_alumno){
		$bd="uclescuelas";
		$mysql= new Connection();
		$esc=null;
		...
		if($mysql->Connect()){
			...
			$query="SELECT mi.data
            from ".$bd.".mdl_user_info_data mi
            where mi.userid = '".$id_moodle_alumno."'
            and mi.fieldid = 13 limit 1";
			$result=$mysql->Query($query);
			if($result['success']){
				while($row=mysqli_fetch_assoc($result['data'])){
					$esc=$row['data'];
		...
		if(isset($_GET['userid']) && is_numeric($_GET['userid'])){
			$id_moodle=$_GET['userid'];
			$escuela=GetEscuelaAlumno($id_moodle);
			if($escuela==null){
				$escuela_nombre=$escuela_nombre;
			}else{
				$id_escuela=explode(" ",$escuela);
				$escuela_nombre.="_".$id_escuela[0];
			}
		}else{
			$escuela_nombre=$escuela_nombre;
		}
    ...
    $params["shownotice"] = 0;  //connects y disconnects
