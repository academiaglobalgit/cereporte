...
app.controller('ctlAdministrador', function($scope,$http,$filter,$window)
{
	...
	//Examen relacion
	$scope.planestudio_item = { id:0 };
	$scope.item_autoridad = {id:0};
	$scope.showEliminarRelacionExamen = false;
	$scope.examenes_relacion = [];
	$scope.relacion_materia = [];
	//Examen relacion
	$scope.mostrarexamenescorporacion = false;
	$scope.pagination = [];
	
	$scope.itemrelacion_eliminar = 0;
	$scope.itemrelacionexamen_eliminar = 0;
	//$scope.showEliminarRelacionMateria = false;
	$scope.btnMostrarRelacionExamenes = false;
	...
	$scope.limpiar = function()
	{
		$scope.planestudio_item = { id:0 };
		$scope.item_autoridad = {id:0};
		//$scope.mostrarexamencorporacion = true;
		$scope.itemrelacionexamen_eliminar = 0;
		$scope.itemrelacion_eliminar = 0;
		$scope.mostrarexamenescorporacion = false;
		$scope.mostrarmateriascorporacion = false;
	...
	$scope.btnMostrarRelacionExamenes_click = function()
	{
		$scope.limpiar();
		$scope.btnMostrarRelacionExamenes = true;

		//GET PLAN DE ESTUDIOS
		//GET MATERIAS AUTORIDAD
		//GET MATERIAS CORPORACIONES
		//GET RELACION DE MATERIAS
		//GET EXAMENES

		$http.get("datos/getPlanEstudios.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
		...
		$http.get("datos/getMaterias.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
				if(angular.isObject(response.data))
		...
		$http.get("datos/getMateriasAutoridad.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
				if(angular.isObject(response.data))
		...
		$http.get("datos/getPlanEstudiosAutoridades.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
				if(angular.isObject(response.data))
		...
		$http.get("datos/getRelacionMateriasAutoridad.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
				if(angular.isObject(response.data))
		...
		$http.get("datos/getExamenes.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
				if(angular.isObject(response.data))
		...
		$http.get("datos/getRelacionExamenes.php").success(function(response,status,headers,config)
		{
			if(response.success)
			{
				if(angular.isObject(response.data))
				{
					$scope.examenes_relacion = response.data;
				}
				else
				{
					//alert("Error de formato json " + response.data);
	...
	//Relación examen
	$scope.btnHideEliminarRelacionExamen_click = function()
	{
		$scope.showEliminarRelacionExamen = false;
	}

	$scope.btnDelItemRelacionExamen_click = function()
	{

		if( $scope.examenes_relacion[$scope.examenes_relacion.indexOf($scope.itemrelacionexamen_eliminar)].id > 0 )
		{
			$http.post("datos/delRelacionExamenAutoridad.php",{"id":$scope.itemrelacionexamen_eliminar.id })
			.success(function(response,status,headers,config)
			{
				if(angular.isObject(response))
				{
					if(response.success)
					{
						$scope.examenes_relacion.splice($scope.examenes_relacion.indexOf($scope.itemrelacionexamen_eliminar),1);
						$scope.mostrarexamenescorporacion = false;
						console.log(response);
					}
					else
	...
	$scope.mostrarexamenes = false;
	$scope.cancelar_relacion_change_examen1 = function(item_plan_estudio)
	{
		$scope.planestudio_item = item_plan_estudio;
		$scope.mostrarexamenes = false;
		$scope.mostrarexamenescorporacion = false;
		$scope.examenes_relacion = $scope.examenes_relacion.filter(function(item){ return item.id  > 0 ;});
	}
	$scope.cancelar_relacion_change_examen = function()
	{
		$scope.mostrarexamenescorporacion = false;
		$scope.examenes_relacion = $scope.examenes_relacion.filter(function(item){ return item.id  > 0 ;});
	}
	$scope.cancelar_relacion_change_examen3 = function(autoridad)
	{
		$scope.item_autoridad = autoridad;
		$scope.mostrarexamenes = false;
		$scope.mostrarexamenescorporacion = false;
		$scope.examenes_relacion = $scope.examenes_relacion.filter(function(item){ return item.id  > 0 ;});
	}
	
	$scope.cancelar_relacion_change_examen2 = function(relacion)
	{
		$scope.mostrarexamenes = true;
		$scope.relacion_materia = relacion;
		$scope.mostrarexamenescorporacion = false;
		$scope.examenes_relacion = $scope.examenes_relacion.filter(function(item){ return item.id  > 0 ;});
	}
	

	$scope.filtro_corporacion = function(filtro)
	{
		//console.log( $scope.item_autoridad.id );
		return filtro.id_plan_estudio == $scope.planestudio_item.id &&  filtro.materia_autoridad.id_plan_estudio_autoridad == $scope.item_autoridad.id  ;
		//return true;

	}
	$scope.btnGuardarExamenRelacion_click = function(examen_item)
	{
		//$scope.examenes_relacion.push( {"id_examen":examen_item.id}  );


		//$scope.mostrarexamencorporacion = (!$scope.mostrarexamencorporacion);
		$scope.mostrarexamenescorporacion = (!$scope.mostrarexamenescorporacion);
		if($scope.mostrarexamenescorporacion)
		{
			
			$scope.examenes_relacion.push( { "id_examen":examen_item.id ,'id_materia_corporacion':$scope.relacion_materia.materia_corporacion.id} );
			
		}
		else
		{
			
			$http.post("datos/addRelacionExamenAutoridad.php", {"id_examen":$scope.examenes_relacion[$scope.examenes_relacion.length - 1].id_examen , "id_examen_autoridad" : examen_item.id }
			).success(function(responser_server,status,headers,config)
			{
				if(angular.isObject(responser_server))
				{
					if(responser_server.success)
					{
						$scope.examenes_relacion[$scope.examenes_relacion.length - 1] = responser_server.data;
					...
					{
						$scope.examenes_relacion[$scope.examenes_relacion.length - 1] = [];
						$scope.mostrarexamenescorporacion = (!$scope.mostrarexamenescorporacion);
					}
				}
				else
	...
	$scope.btnShowEliminarRelacionExamen_click = function(relacion_item)
	{
		if(relacion_item.id > 0)
		{
			$scope.itemrelacionexamen_eliminar = relacion_item;
			$scope.showEliminarRelacionExamen= (!$scope.showEliminarRelacionExamen);

		}
		else
		{
			$scope.examenes_relacion.splice($scope.examenes_relacion.indexOf(relacion_item),1);
			$scope.mostrarexamenescorporacion = false;
		}
	}
	...
	//Relación examen
	$scope.btnMostrarRelacionExamenes_click();
...
//Examen relacion
app.directive('modalexamenrelacion', function ()
{
    return {
      template: '<div class="modal fade">' +
          '<div class="modal-dialog modal-lg">' +
            '<div class="modal-content">' +
              '<div class="modal-header">' +
                '<button type="button" class="close" data-dismiss="modal" aria-hidden="true" ng-click="btnHideEliminarRelacionExamen_click()">&times;</button>' +
                '<h4 class="modal-title" ng-bind="title"></h4>' +
              '</div>' +
              '<div class="modal-body" ng-transclude></div>' +
            '</div>' +
